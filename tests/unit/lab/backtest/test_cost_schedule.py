...
def _sample_schedule_yaml(extra_interval: str = "") -> str:
    return (
        ...
        "    precision: 0\n"
        "    sources:\n"
        "      - 'Indodax Fee Schedule 2024'\n"
        f"{extra_interval}"
    )


def test_cost_01_valid_contract() -> None:
    ...
    event_ts = datetime(2023, 6, 15, 12, 0, 0, tzinfo=UTC)
    res_buy = lookup_cost(
        table,
        market="spot_idr",
        side=OrderSide.BUY,
    ...
    assert res_buy.valid_from <= event_ts
    assert res_buy.valid_to is None or event_ts < res_buy.valid_to
    ...
    assert res_now.side is OrderSide.SELL
...
def test_cost_01_contract_1(tmp_path: Path) -> None:
    ...
    overlap_file.write_text(_sample_schedule_yaml(extra_interval=overlap_interval), encoding="utf-8")
...
def test_cost_schedule_rejects_negative_rate_and_naive_datetime(tmp_path: Path) -> None:
    """Rates must be nonnegative and event_ts must be timezone-aware UTC."""
    negative_file = tmp_path / "negative.yaml"
    negative_file.write_text(
        _sample_schedule_yaml().replace("service_fee_rate: '0.002000'", "service_fee_rate: '-0.001'"),
        encoding="utf-8",
    )
    ...
    with pytest.raises(ValueError, match="UTC_TIMEZONE_AWARE_REQUIRED"):
        lookup_cost(table, market="spot_idr", side=OrderSide.BUY, role=OrderRole.TAKER, event_ts=naive_ts)
