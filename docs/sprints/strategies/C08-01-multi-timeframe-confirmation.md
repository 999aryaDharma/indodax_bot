...
Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C08-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Closed daily/4h trend and lower timeframe trigger as-of decision -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C08-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C08-01-FR0:** Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C08-01-FR1:** Partial daily trend ditolak.
2. **C08-01-FR2:** Stale higher timeframe abstain.
3. **C08-01-FR3:** Confirmed alignment memberi LONG.
...
- `configs/strategies/C08_v1.yaml`
- `src/indodax_lab/strategies/c08.py`
- `tests/unit/lab/strategies/test_c08.py`
...
First establish C08-01-AC0: Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C08-01-AC1`, build minimal fixture proving: Partial daily trend ditolak. Write `test_c08_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C08-01-AC2`, build minimal fixture proving: Stale higher timeframe abstain. Write `test_c08_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C08-01-AC3`, build minimal fixture proving: Confirmed alignment memberi LONG. Write `test_c08_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C08-01-AC0**, `test_c08_01_valid_contract` — Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C08-01-AC1 | `test_c08_01_contract_1` | Partial daily trend ditolak |
| C08-01-AC2 | `test_c08_01_contract_2` | Stale higher timeframe abstain |
| C08-01-AC3 | `test_c08_01_contract_3` | Confirmed alignment memberi LONG |
...
- Case 1: Partial daily trend ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Stale higher timeframe abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Confirmed alignment memberi LONG. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C08-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C08-01-AC0** Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C08-01-AC1** Partial daily trend ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C08-01-AC2** Stale higher timeframe abstain. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C08-01-AC3** Confirmed alignment memberi LONG. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Partial daily trend ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Stale higher timeframe abstain. Inspect fixture and actual production path.
- Attempt to disprove: Confirmed alignment memberi LONG. Inspect fixture and actual production path.
...
Branch: `feat/c08-01-multi-timeframe-confirmation`. Commit: `feat(c08-01): multi timeframe confirmation` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C08-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C08-01 only: Multi timeframe confirmation.
Read AGENTS.md, docs/sprints/strategies/C08-01-multi-timeframe-confirmation.md and every Required Reading path listed in it.
...
Goal: Kandidat C08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Closed daily/4h trend and lower timeframe trigger as-of decision -> versioned LONG/FLAT intent, never direct orders.
