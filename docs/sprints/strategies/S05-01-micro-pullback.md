...
Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `S05-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Short pullback within liquid impulse with spread-aware timing -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S05-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S05-01-FR0:** Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S05-01-FR1:** Wide spread menolak.
2. **S05-01-FR2:** Price chase di luar band menolak.
3. **S05-01-FR3:** Partial fill tidak menjadi full size.
...
- `configs/strategies/S05_v1.yaml`
- `src/indodax_lab/strategies/s05.py`
- `tests/unit/lab/strategies/test_s05.py`
...
First establish S05-01-AC0: Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S05-01-AC1`, build minimal fixture proving: Wide spread menolak. Write `test_s05_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S05-01-AC2`, build minimal fixture proving: Price chase di luar band menolak. Write `test_s05_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S05-01-AC3`, build minimal fixture proving: Partial fill tidak menjadi full size. Write `test_s05_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S05-01-AC0**, `test_s05_01_valid_contract` — Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S05-01-AC1 | `test_s05_01_contract_1` | Wide spread menolak |
| S05-01-AC2 | `test_s05_01_contract_2` | Price chase di luar band menolak |
| S05-01-AC3 | `test_s05_01_contract_3` | Partial fill tidak menjadi full size |
...
- Case 2: Price chase di luar band menolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Partial fill tidak menjadi full size. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S05-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S05-01-AC0** Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **S05-01-AC1** Wide spread menolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S05-01-AC2** Price chase di luar band menolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S05-01-AC3** Partial fill tidak menjadi full size. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Price chase di luar band menolak. Inspect fixture and actual production path.
- Attempt to disprove: Partial fill tidak menjadi full size. Inspect fixture and actual production path.
...
Branch: `feat/s05-01-micro-pullback`. Commit: `feat(s05-01): micro pullback` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S05-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S05-01 only: Micro pullback.
Read AGENTS.md, docs/sprints/strategies/S05-01-micro-pullback.md and every Required Reading path listed in it.
...
Goal: Kandidat S05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Short pullback within liquid impulse with spread-aware timing -> versioned LONG/FLAT intent, never direct orders.
