...
Recommended Branch: `feat/s07-01-small-cap-rotation`
...
Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `S07-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Liquidity capacity constrained cross-sectional rank turnover -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S07-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S07-01-FR0:** Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S07-01-FR1:** Illiquid top rank tidak otomatis eligible.
2. **S07-01-FR2:** Order di atas capacity ditolak.
3. **S07-01-FR3:** Rotation respects shared cash.
...
- `configs/strategies/S07_v1.yaml`
- `src/indodax_lab/strategies/s07.py`
- `tests/unit/lab/strategies/test_s07.py`
...
First establish S07-01-AC0: Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S07-01-AC1`, build minimal fixture proving: Illiquid top rank tidak otomatis eligible. Write `test_s07_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S07-01-AC2`, build minimal fixture proving: Order di atas capacity ditolak. Write `test_s07_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S07-01-AC3`, build minimal fixture proving: Rotation respects shared cash. Write `test_s07_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S07-01-AC0**, `test_s07_01_valid_contract` — Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S07-01-AC1 | `test_s07_01_contract_1` | Illiquid top rank tidak otomatis eligible |
| S07-01-AC2 | `test_s07_01_contract_2` | Order di atas capacity ditolak |
| S07-01-AC3 | `test_s07_01_contract_3` | Rotation respects shared cash |
...
- Case 1: Illiquid top rank tidak otomatis eligible. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Order di atas capacity ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Rotation respects shared cash. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S07-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S07-01-AC0** Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **S07-01-AC1** Illiquid top rank tidak otomatis eligible. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S07-01-AC2** Order di atas capacity ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S07-01-AC3** Rotation respects shared cash. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Illiquid top rank tidak otomatis eligible. Inspect fixture and actual production path.
- Attempt to disprove: Order di atas capacity ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Rotation respects shared cash. Inspect fixture and actual production path.
...
Branch: `feat/s07-01-small-cap-rotation`. Commit: `feat(s07-01): small cap rotation` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S07-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S07-01 only: Small cap rotation.
Read AGENTS.md, docs/sprints/strategies/S07-01-small-cap-rotation.md and every Required Reading path listed in it.
...
Goal: Kandidat S07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Liquidity capacity constrained cross-sectional rank turnover -> versioned LONG/FLAT intent, never direct orders.
