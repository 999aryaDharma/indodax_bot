...
Kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C05-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Prior contraction then range expansion with registered ATR risk -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C05-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C05-01-FR0:** Kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C05-01-FR1:** Future range tidak membentuk contraction.
2. **C05-01-FR2:** Expansion memberi intent.
3. **C05-01-FR3:** Degenerate range abstain.
...
- `configs/strategies/C05_v1.yaml`
- `src/indodax_lab/strategies/c05.py`
- `tests/unit/lab/strategies/test_c05.py`
...
3. For `C05-01-AC2`, build minimal fixture proving: Expansion memberi intent. Write `test_c05_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C05-01-AC3`, build minimal fixture proving: Degenerate range abstain. Write `test_c05_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C05-01-AC0**, `test_c05_01_valid_contract` — Kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C05-01-AC1 | `test_c05_01_contract_1` | Future range tidak membentuk contraction |
| C05-01-AC2 | `test_c05_01_contract_2` | Expansion memberi intent |
| C05-01-AC3 | `test_c05_01_contract_3` | Degenerate range abstain |
...
- Case 1: Future range tidak membentuk contraction. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Expansion memberi intent. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Degenerate range abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C05-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C05-01-AC0** Kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C05-01-AC1** Future range tidak membentuk contraction. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C05-01-AC2** Expansion memberi intent. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C05-01-AC3** Degenerate range abstain. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Future range tidak membentuk contraction. Inspect fixture and actual production path.
- Attempt to disprove: Expansion memberi intent. Inspect fixture and actual production path.
- Attempt to disprove: Degenerate range abstain. Inspect fixture and actual production path.
...
Branch: `feat/c05-01-volatility-breakout`. Commit: `feat(c05-01): volatility breakout` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C05-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C05-01 only: Volatility breakout.
Read AGENTS.md, docs/sprints/strategies/C05-01-volatility-breakout.md and every Required Reading path listed in it.
...
Goal: Kandidat C05 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Prior contraction then range expansion with registered ATR risk -> versioned LONG/FLAT intent, never direct orders.
