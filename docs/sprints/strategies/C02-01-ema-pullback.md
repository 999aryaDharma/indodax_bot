...
Recommended Branch: `feat/c02-01-ema-pullback`
...
Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C02-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Long EMA regime plus short EMA pullback and recovery -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C02-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C02-01-FR0:** Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C02-01-FR1:** Downtrend menolak buy.
2. **C02-01-FR2:** Pullback belum recovered tidak entry.
3. **C02-01-FR3:** Recovery closed bar memberi intent.
...
- `configs/strategies/C02_v1.yaml`
- `src/indodax_lab/strategies/c02.py`
...
First establish C02-01-AC0: Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C02-01-AC1`, build minimal fixture proving: Downtrend menolak buy. Write `test_c02_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C02-01-AC2`, build minimal fixture proving: Pullback belum recovered tidak entry. Write `test_c02_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C02-01-AC3`, build minimal fixture proving: Recovery closed bar memberi intent. Write `test_c02_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C02-01-AC0**, `test_c02_01_valid_contract` — Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C02-01-AC1 | `test_c02_01_contract_1` | Downtrend menolak buy |
| C02-01-AC2 | `test_c02_01_contract_2` | Pullback belum recovered tidak entry |
| C02-01-AC3 | `test_c02_01_contract_3` | Recovery closed bar memberi intent |
...
- Case 1: Downtrend menolak buy. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Pullback belum recovered tidak entry. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Recovery closed bar memberi intent. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C02-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C02-01-AC0** Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C02-01-AC1** Downtrend menolak buy. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C02-01-AC2** Pullback belum recovered tidak entry. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C02-01-AC3** Recovery closed bar memberi intent. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Downtrend menolak buy. Inspect fixture and actual production path.
- Attempt to disprove: Pullback belum recovered tidak entry. Inspect fixture and actual production path.
- Attempt to disprove: Recovery closed bar memberi intent. Inspect fixture and actual production path.
...
Branch: `feat/c02-01-ema-pullback`. Commit: `feat(c02-01): ema pullback` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C02-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C02-01 only: EMA pullback.
Read AGENTS.md, docs/sprints/strategies/C02-01-ema-pullback.md and every Required Reading path listed in it.
...
Goal: Kandidat C02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Long EMA regime plus short EMA pullback and recovery -> versioned LONG/FLAT intent, never direct orders.
