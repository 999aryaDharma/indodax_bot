...
Recommended Branch: `feat/c03-01-time-series-momentum`
...
Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C03-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Positive lookback return with volatility target and cash fallback -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C03-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C03-01-FR0:** Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C03-01-FR1:** Negative momentum tetap cash.
2. **C03-01-FR2:** Zero volatility memberi reject.
3. **C03-01-FR3:** Future return tidak mengubah sizing.
...
- `configs/strategies/C03_v1.yaml`
- `src/indodax_lab/strategies/c03.py`
- `tests/unit/lab/strategies/test_c03.py`
...
First establish C03-01-AC0: Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C03-01-AC1`, build minimal fixture proving: Negative momentum tetap cash. Write `test_c03_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C03-01-AC2`, build minimal fixture proving: Zero volatility memberi reject. Write `test_c03_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C03-01-AC3`, build minimal fixture proving: Future return tidak mengubah sizing. Write `test_c03_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C03-01-AC0**, `test_c03_01_valid_contract` — Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C03-01-AC1 | `test_c03_01_contract_1` | Negative momentum tetap cash |
| C03-01-AC2 | `test_c03_01_contract_2` | Zero volatility memberi reject |
| C03-01-AC3 | `test_c03_01_contract_3` | Future return tidak mengubah sizing |
...
- Case 1: Negative momentum tetap cash. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Zero volatility memberi reject. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Future return tidak mengubah sizing. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C03-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C03-01-AC0** Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C03-01-AC1** Negative momentum tetap cash. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C03-01-AC2** Zero volatility memberi reject. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C03-01-AC3** Future return tidak mengubah sizing. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Negative momentum tetap cash. Inspect fixture and actual production path.
- Attempt to disprove: Zero volatility memberi reject. Inspect fixture and actual production path.
- Attempt to disprove: Future return tidak mengubah sizing. Inspect fixture and actual production path.
...
Branch: `feat/c03-01-time-series-momentum`. Commit: `feat(c03-01): time series momentum` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C03-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C03-01 only: Time series momentum.
Read AGENTS.md, docs/sprints/strategies/C03-01-time-series-momentum.md and every Required Reading path listed in it.
...
Goal: Kandidat C03 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Positive lookback return with volatility target and cash fallback -> versioned LONG/FLAT intent, never direct orders.
