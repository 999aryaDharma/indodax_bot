...
Kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C01-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Previous N-bar high breakout with volume gate; ATR stop -> versioned LONG/FLAT intent, never direct orders.

Direct consumers: C10-01, QA-01, M05-01
...
- Kapabilitas downstream: C10-01, QA-01, M05-01.
...
Given input yang memenuhi kontrak `C01-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C01-01-FR0:** Kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C01-01-FR1:** Current bar tidak ikut previous high.
2. **C01-01-FR2:** Breakout confirmed menghasilkan LONG.
3. **C01-01-FR3:** Incomplete bar menghasilkan FLAT.
...
- `configs/strategies/C01_v1.yaml`
- `src/indodax_lab/strategies/c01.py`
- `tests/unit/lab/strategies/test_c01.py`
...
First establish C01-01-AC0: Kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C01-01-AC1`, build minimal fixture proving: Current bar tidak ikut previous high. Write `test_c01_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C01-01-AC2`, build minimal fixture proving: Breakout confirmed menghasilkan LONG. Write `test_c01_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C01-01-AC3`, build minimal fixture proving: Incomplete bar menghasilkan FLAT. Write `test_c01_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C01-01-AC0**, `test_c01_01_valid_contract` — Kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C01-01-AC1 | `test_c01_01_contract_1` | Current bar tidak ikut previous high |
| C01-01-AC2 | `test_c01_01_contract_2` | Breakout confirmed menghasilkan LONG |
| C01-01-AC3 | `test_c01_01_contract_3` | Incomplete bar menghasilkan FLAT |
...
- Case 1: Current bar tidak ikut previous high. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Breakout confirmed menghasilkan LONG. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Incomplete bar menghasilkan FLAT. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C01-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- Attempt to disprove: Current bar tidak ikut previous high. Inspect fixture and actual production path.
- Attempt to disprove: Breakout confirmed menghasilkan LONG. Inspect fixture and actual production path.
- Attempt to disprove: Incomplete bar menghasilkan FLAT. Inspect fixture and actual production path.
...
Branch: `feat/c01-01-donchian-breakout`. Commit: `feat(c01-01): donchian breakout` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C01-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: C10-01, QA-01, M05-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C01-01 only: Donchian breakout.
Read AGENTS.md, docs/sprints/strategies/C01-01-donchian-breakout.md and every Required Reading path listed in it.
...
Goal: Kandidat C01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Previous N-bar high breakout with volume gate; ATR stop -> versioned LONG/FLAT intent, never direct orders.
