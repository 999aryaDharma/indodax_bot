...
Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C06-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: ADX strength and signed DI with ATR protection -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C06-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C06-01-FR0:** Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C06-01-FR1:** High ADX negative DI tidak buy.
2. **C06-01-FR2:** Warmup belum lengkap abstain.
3. **C06-01-FR3:** Threshold equality deterministik.
...
- `configs/strategies/C06_v1.yaml`
- `src/indodax_lab/strategies/c06.py`
- `tests/unit/lab/strategies/test_c06.py`
...
First establish C06-01-AC0: Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C06-01-AC1`, build minimal fixture proving: High ADX negative DI tidak buy. Write `test_c06_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C06-01-AC2`, build minimal fixture proving: Warmup belum lengkap abstain. Write `test_c06_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C06-01-AC3`, build minimal fixture proving: Threshold equality deterministik. Write `test_c06_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C06-01-AC0**, `test_c06_01_valid_contract` — Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C06-01-AC1 | `test_c06_01_contract_1` | High ADX negative DI tidak buy |
| C06-01-AC2 | `test_c06_01_contract_2` | Warmup belum lengkap abstain |
| C06-01-AC3 | `test_c06_01_contract_3` | Threshold equality deterministik |
...
- Case 1: High ADX negative DI tidak buy. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Warmup belum lengkap abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Threshold equality deterministik. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C06-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C06-01-AC0** Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C06-01-AC1** High ADX negative DI tidak buy. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C06-01-AC2** Warmup belum lengkap abstain. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C06-01-AC3** Threshold equality deterministik. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: High ADX negative DI tidak buy. Inspect fixture and actual production path.
- Attempt to disprove: Warmup belum lengkap abstain. Inspect fixture and actual production path.
- Attempt to disprove: Threshold equality deterministik. Inspect fixture and actual production path.
...
Branch: `feat/c06-01-directional-trend-strength`. Commit: `feat(c06-01): directional trend strength` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C06-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C06-01 only: Directional trend strength.
Read AGENTS.md, docs/sprints/strategies/C06-01-directional-trend-strength.md and every Required Reading path listed in it.
...
Goal: Kandidat C06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: ADX strength and signed DI with ATR protection -> versioned LONG/FLAT intent, never direct orders.
