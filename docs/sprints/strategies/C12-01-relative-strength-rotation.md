...
Recommended Branch: `feat/c12-01-relative-strength-rotation`
...
Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C12-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: PIT strength rotation with turnover and cash regime gate -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C12-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C12-01-FR0:** Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C12-01-FR1:** Delisted pair tetap ada dalam histori.
2. **C12-01-FR2:** Cash regime memblokir reentry.
3. **C12-01-FR3:** Tie rank stable.
...
- `configs/strategies/C12_v1.yaml`
- `src/indodax_lab/strategies/c12.py`
- `tests/unit/lab/strategies/test_c12.py`
...
First establish C12-01-AC0: Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C12-01-AC1`, build minimal fixture proving: Delisted pair tetap ada dalam histori. Write `test_c12_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C12-01-AC2`, build minimal fixture proving: Cash regime memblokir reentry. Write `test_c12_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C12-01-AC3`, build minimal fixture proving: Tie rank stable. Write `test_c12_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C12-01-AC0**, `test_c12_01_valid_contract` — Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C12-01-AC1 | `test_c12_01_contract_1` | Delisted pair tetap ada dalam histori |
| C12-01-AC2 | `test_c12_01_contract_2` | Cash regime memblokir reentry |
| C12-01-AC3 | `test_c12_01_contract_3` | Tie rank stable |
...
- Case 1: Delisted pair tetap ada dalam histori. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Cash regime memblokir reentry. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Tie rank stable. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C12-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C12-01-AC0** Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C12-01-AC1** Delisted pair tetap ada dalam histori. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C12-01-AC2** Cash regime memblokir reentry. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C12-01-AC3** Tie rank stable. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Delisted pair tetap ada dalam histori. Inspect fixture and actual production path.
- Attempt to disprove: Cash regime memblokir reentry. Inspect fixture and actual production path.
- Attempt to disprove: Tie rank stable. Inspect fixture and actual production path.
...
Branch: `feat/c12-01-relative-strength-rotation`. Commit: `feat(c12-01): relative strength rotation` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C12-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C12-01 only: Relative strength rotation.
Read AGENTS.md, docs/sprints/strategies/C12-01-relative-strength-rotation.md and every Required Reading path listed in it.
...
Goal: Kandidat C12 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: PIT strength rotation with turnover and cash regime gate -> versioned LONG/FLAT intent, never direct orders.
