...
Tanpa kapabilitas ini, kontrak `S06-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Momentum only after registered minimum age and reliable history -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S06-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S06-01-FR0:** Kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S06-01-FR1:** Initial listing spike ditolak.
2. **S06-01-FR2:** Age boundary memakai available listing evidence.
3. **S06-01-FR3:** Unknown listing date abstain.
...
- `configs/strategies/S06_v1.yaml`
- `src/indodax_lab/strategies/s06.py`
- `tests/unit/lab/strategies/test_s06.py`
...
First establish S06-01-AC0: Kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S06-01-AC1`, build minimal fixture proving: Initial listing spike ditolak. Write `test_s06_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S06-01-AC2`, build minimal fixture proving: Age boundary memakai available listing evidence. Write `test_s06_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S06-01-AC3`, build minimal fixture proving: Unknown listing date abstain. Write `test_s06_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S06-01-AC0**, `test_s06_01_valid_contract` — Kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S06-01-AC1 | `test_s06_01_contract_1` | Initial listing spike ditolak |
| S06-01-AC2 | `test_s06_01_contract_2` | Age boundary memakai available listing evidence |
| S06-01-AC3 | `test_s06_01_contract_3` | Unknown listing date abstain |
...
- Case 1: Initial listing spike ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Age boundary memakai available listing evidence. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Unknown listing date abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S06-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S06-01-AC0** Kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **S06-01-AC1** Initial listing spike ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S06-01-AC2** Age boundary memakai available listing evidence. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S06-01-AC3** Unknown listing date abstain. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Initial listing spike ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Age boundary memakai available listing evidence. Inspect fixture and actual production path.
- Attempt to disprove: Unknown listing date abstain. Inspect fixture and actual production path.
...
Branch: `feat/s06-01-post-listing-maturation`. Commit: `feat(s06-01): post listing maturation` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S06-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S06-01 only: Post listing maturation.
Read AGENTS.md, docs/sprints/strategies/S06-01-post-listing-maturation.md and every Required Reading path listed in it.
...
Goal: Kandidat S06 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Momentum only after registered minimum age and reliable history -> versioned LONG/FLAT intent, never direct orders.
