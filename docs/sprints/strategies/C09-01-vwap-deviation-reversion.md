...
Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C09-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Rolling VWAP deviation in eligible reversion regime -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C09-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C09-01-FR0:** Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C09-01-FR1:** Zero denominator abstain.
2. **C09-01-FR2:** Trend risk gate menolak falling price.
3. **C09-01-FR3:** Missing volume tidak diisi nol.
...
- `configs/strategies/C09_v1.yaml`
- `src/indodax_lab/strategies/c09.py`
- `tests/unit/lab/strategies/test_c09.py`
...
First establish C09-01-AC0: Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C09-01-AC1`, build minimal fixture proving: Zero denominator abstain. Write `test_c09_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C09-01-AC2`, build minimal fixture proving: Trend risk gate menolak falling price. Write `test_c09_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C09-01-AC3`, build minimal fixture proving: Missing volume tidak diisi nol. Write `test_c09_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C09-01-AC0**, `test_c09_01_valid_contract` — Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C09-01-AC1 | `test_c09_01_contract_1` | Zero denominator abstain |
| C09-01-AC2 | `test_c09_01_contract_2` | Trend risk gate menolak falling price |
| C09-01-AC3 | `test_c09_01_contract_3` | Missing volume tidak diisi nol |
...
- Case 1: Zero denominator abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Trend risk gate menolak falling price. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Missing volume tidak diisi nol. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C09-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C09-01-AC0** Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C09-01-AC1** Zero denominator abstain. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C09-01-AC2** Trend risk gate menolak falling price. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C09-01-AC3** Missing volume tidak diisi nol. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Zero denominator abstain. Inspect fixture and actual production path.
- Attempt to disprove: Trend risk gate menolak falling price. Inspect fixture and actual production path.
- Attempt to disprove: Missing volume tidak diisi nol. Inspect fixture and actual production path.
...
Branch: `feat/c09-01-vwap-deviation-reversion`. Commit: `feat(c09-01): vwap deviation reversion` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C09-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C09-01 only: VWAP deviation reversion.
Read AGENTS.md, docs/sprints/strategies/C09-01-vwap-deviation-reversion.md and every Required Reading path listed in it.
...
Goal: Kandidat C09 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Rolling VWAP deviation in eligible reversion regime -> versioned LONG/FLAT intent, never direct orders.
