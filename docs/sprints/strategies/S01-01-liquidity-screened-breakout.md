...
Recommended Branch: `feat/s01-01-liquidity-screened-breakout`
...
Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `S01-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Breakout with spread/depth gate relative to simulated size -> versioned LONG/FLAT intent, never direct orders.

Direct consumers: S07-01, QA-01
...
- Kapabilitas downstream: S07-01, QA-01.
...
Given input yang memenuhi kontrak `S01-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S01-01-FR0:** Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S01-01-FR1:** Wide spread menolak.
2. **S01-01-FR2:** Depth kurang membatasi size.
3. **S01-01-FR3:** Missing liquidity memblokir trade.
...
- `configs/strategies/S01_v1.yaml`
- `src/indodax_lab/strategies/s01.py`
- `tests/unit/lab/strategies/test_s01.py`
...
First establish S01-01-AC0: Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S01-01-AC1`, build minimal fixture proving: Wide spread menolak. Write `test_s01_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S01-01-AC2`, build minimal fixture proving: Depth kurang membatasi size. Write `test_s01_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S01-01-AC3`, build minimal fixture proving: Missing liquidity memblokir trade. Write `test_s01_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S01-01-AC0**, `test_s01_01_valid_contract` — Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S01-01-AC1 | `test_s01_01_contract_1` | Wide spread menolak |
| S01-01-AC2 | `test_s01_01_contract_2` | Depth kurang membatasi size |
| S01-01-AC3 | `test_s01_01_contract_3` | Missing liquidity memblokir trade |
...
- Case 1: Wide spread menolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Depth kurang membatasi size. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Missing liquidity memblokir trade. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S01-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S01-01-AC0** Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **S01-01-AC1** Wide spread menolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S01-01-AC2** Depth kurang membatasi size. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S01-01-AC3** Missing liquidity memblokir trade. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Wide spread menolak. Inspect fixture and actual production path.
- Attempt to disprove: Depth kurang membatasi size. Inspect fixture and actual production path.
- Attempt to disprove: Missing liquidity memblokir trade. Inspect fixture and actual production path.
...
Branch: `feat/s01-01-liquidity-screened-breakout`. Commit: `feat(s01-01): liquidity screened breakout` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S01-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: S07-01, QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S01-01 only: Liquidity screened breakout.
Read AGENTS.md, docs/sprints/strategies/S01-01-liquidity-screened-breakout.md and every Required Reading path listed in it.
...
Goal: Kandidat S01 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Breakout with spread/depth gate relative to simulated size -> versioned LONG/FLAT intent, never direct orders.
