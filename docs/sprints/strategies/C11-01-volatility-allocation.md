...
Recommended Branch: `feat/c11-01-volatility-allocation`
...
Kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C11-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Inverse volatility long-only weights with asset and cash constraints -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `C11-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C11-01-FR0:** Kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C11-01-FR1:** Zero volatility tidak mendapat infinite weight.
2. **C11-01-FR2:** Sum allocation tidak melebihi cash.
3. **C11-01-FR3:** Small universe fallback explicit.
...
- `configs/strategies/C11_v1.yaml`
- `src/indodax_lab/strategies/c11.py`
- `tests/unit/lab/strategies/test_c11.py`
...
First establish C11-01-AC0: Kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C11-01-AC1`, build minimal fixture proving: Zero volatility tidak mendapat infinite weight. Write `test_c11_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C11-01-AC2`, build minimal fixture proving: Sum allocation tidak melebihi cash. Write `test_c11_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C11-01-AC3`, build minimal fixture proving: Small universe fallback explicit. Write `test_c11_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C11-01-AC0**, `test_c11_01_valid_contract` — Kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C11-01-AC1 | `test_c11_01_contract_1` | Zero volatility tidak mendapat infinite weight |
| C11-01-AC2 | `test_c11_01_contract_2` | Sum allocation tidak melebihi cash |
| C11-01-AC3 | `test_c11_01_contract_3` | Small universe fallback explicit |
...
- Case 1: Zero volatility tidak mendapat infinite weight. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Sum allocation tidak melebihi cash. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Small universe fallback explicit. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C11-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- Attempt to disprove: Zero volatility tidak mendapat infinite weight. Inspect fixture and actual production path.
- Attempt to disprove: Sum allocation tidak melebihi cash. Inspect fixture and actual production path.
- Attempt to disprove: Small universe fallback explicit. Inspect fixture and actual production path.
...
Branch: `feat/c11-01-volatility-allocation`. Commit: `feat(c11-01): volatility allocation` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C11-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C11-01 only: Volatility allocation.
Read AGENTS.md, docs/sprints/strategies/C11-01-volatility-allocation.md and every Required Reading path listed in it.
...
Goal: Kandidat C11 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Inverse volatility long-only weights with asset and cash constraints -> versioned LONG/FLAT intent, never direct orders.
