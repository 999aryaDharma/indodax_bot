...
Kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `S04-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Reliable-session trade/book imbalance over registered horizon -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S04-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S04-01-FR0:** Kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S04-01-FR1:** Sequence gap abstain.
2. **S04-01-FR2:** Stale book abstain.
3. **S04-01-FR3:** Imbalance timestamp setelah decision ditolak.
...
- `configs/strategies/S04_v1.yaml`
- `src/indodax_lab/strategies/s04.py`
- `tests/unit/lab/strategies/test_s04.py`
...
First establish S04-01-AC0: Kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S04-01-AC1`, build minimal fixture proving: Sequence gap abstain. Write `test_s04_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S04-01-AC2`, build minimal fixture proving: Stale book abstain. Write `test_s04_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S04-01-AC3`, build minimal fixture proving: Imbalance timestamp setelah decision ditolak. Write `test_s04_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S04-01-AC0**, `test_s04_01_valid_contract` — Kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S04-01-AC1 | `test_s04_01_contract_1` | Sequence gap abstain |
| S04-01-AC2 | `test_s04_01_contract_2` | Stale book abstain |
| S04-01-AC3 | `test_s04_01_contract_3` | Imbalance timestamp setelah decision ditolak |
...
- Case 1: Sequence gap abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Stale book abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Imbalance timestamp setelah decision ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S04-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- Attempt to disprove: Sequence gap abstain. Inspect fixture and actual production path.
- Attempt to disprove: Stale book abstain. Inspect fixture and actual production path.
- Attempt to disprove: Imbalance timestamp setelah decision ditolak. Inspect fixture and actual production path.
...
Branch: `feat/s04-01-order-flow-imbalance`. Commit: `feat(s04-01): order flow imbalance` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S04-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S04-01 only: Order flow imbalance.
Read AGENTS.md, docs/sprints/strategies/S04-01-order-flow-imbalance.md and every Required Reading path listed in it.
...
Goal: Kandidat S04 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Reliable-session trade/book imbalance over registered horizon -> versioned LONG/FLAT intent, never direct orders.
