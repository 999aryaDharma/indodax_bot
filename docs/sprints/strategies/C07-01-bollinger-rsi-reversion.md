...
Recommended Branch: `feat/c07-01-bollinger-rsi-reversion`
...
Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C07-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Extreme BB and RSI deviation only under available sideways regime -> versioned LONG/FLAT intent, never direct orders.

Direct consumers: C10-01, QA-01
...
- Kapabilitas downstream: C10-01, QA-01.
...
Given input yang memenuhi kontrak `C07-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C07-01-FR0:** Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C07-01-FR1:** Strong downtrend menolak entry.
2. **C07-01-FR2:** Sideways oversold memberi bounded intent.
3. **C07-01-FR3:** Zero band width memberi abstain.
...
- `configs/strategies/C07_v1.yaml`
- `src/indodax_lab/strategies/c07.py`
...
First establish C07-01-AC0: Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C07-01-AC1`, build minimal fixture proving: Strong downtrend menolak entry. Write `test_c07_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C07-01-AC2`, build minimal fixture proving: Sideways oversold memberi bounded intent. Write `test_c07_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C07-01-AC3`, build minimal fixture proving: Zero band width memberi abstain. Write `test_c07_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C07-01-AC0**, `test_c07_01_valid_contract` — Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C07-01-AC1 | `test_c07_01_contract_1` | Strong downtrend menolak entry |
| C07-01-AC2 | `test_c07_01_contract_2` | Sideways oversold memberi bounded intent |
| C07-01-AC3 | `test_c07_01_contract_3` | Zero band width memberi abstain |
...
- Case 1: Strong downtrend menolak entry. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Sideways oversold memberi bounded intent. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Zero band width memberi abstain. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C07-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C07-01-AC0** Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C07-01-AC1** Strong downtrend menolak entry. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C07-01-AC2** Sideways oversold memberi bounded intent. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C07-01-AC3** Zero band width memberi abstain. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Strong downtrend menolak entry. Inspect fixture and actual production path.
- Attempt to disprove: Sideways oversold memberi bounded intent. Inspect fixture and actual production path.
- Attempt to disprove: Zero band width memberi abstain. Inspect fixture and actual production path.
...
Branch: `feat/c07-01-bollinger-rsi-reversion`. Commit: `feat(c07-01): bollinger rsi reversion` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C07-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: C10-01, QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C07-01 only: Bollinger RSI reversion.
Read AGENTS.md, docs/sprints/strategies/C07-01-bollinger-rsi-reversion.md and every Required Reading path listed in it.
...
Goal: Kandidat C07 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Extreme BB and RSI deviation only under available sideways regime -> versioned LONG/FLAT intent, never direct orders.
