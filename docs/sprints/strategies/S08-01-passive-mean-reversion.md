...
Tanpa kapabilitas ini, kontrak `S08-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Conservative queue-based passive order simulation without guaranteed maker fills -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S08-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S08-01-FR0:** Kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
- `configs/strategies/S08_v1.yaml`
- `src/indodax_lab/strategies/s08.py`
- `tests/unit/lab/strategies/test_s08.py`
...
First establish S08-01-AC0: Kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S08-01-AC1`, build minimal fixture proving: Price touches quote tidak otomatis fill. Write `test_s08_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S08-01-AC2`, build minimal fixture proving: Queue unknown memblokir promotion. Write `test_s08_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S08-01-AC3`, build minimal fixture proving: Partial cancel menjaga ledger balance. Write `test_s08_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S08-01-AC0**, `test_s08_01_valid_contract` — Kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.
...
- Case 1: Price touches quote tidak otomatis fill. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Queue unknown memblokir promotion. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Partial cancel menjaga ledger balance. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S08-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S08-01-AC0** Kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
...
- Attempt to disprove: Price touches quote tidak otomatis fill. Inspect fixture and actual production path.
- Attempt to disprove: Queue unknown memblokir promotion. Inspect fixture and actual production path.
- Attempt to disprove: Partial cancel menjaga ledger balance. Inspect fixture and actual production path.
...
Branch: `feat/s08-01-passive-mean-reversion`. Commit: `feat(s08-01): passive mean reversion` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S08-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S08-01 only: Passive mean reversion.
Read AGENTS.md, docs/sprints/strategies/S08-01-passive-mean-reversion.md and every Required Reading path listed in it.
...
Goal: Kandidat S08 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Conservative queue-based passive order simulation without guaranteed maker fills -> versioned LONG/FLAT intent, never direct orders.
