...
Recommended Branch: `feat/c10-01-regime-ensemble`
...
Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `C10-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Deterministic trend/reversion switch using available regime and frozen members -> versioned LONG/FLAT intent, never direct orders.

Direct consumers: QA-01
...
Given input yang memenuhi kontrak `C10-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **C10-01-FR0:** Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **C10-01-FR1:** Unknown regime menghasilkan cash.
2. **C10-01-FR2:** Perubahan future regime tidak mengubah decision.
3. **C10-01-FR3:** Member version tercatat di intent.
...
- `configs/strategies/C10_v1.yaml`
- `src/indodax_lab/strategies/c10.py`
- `tests/unit/lab/strategies/test_c10.py`
...
First establish C10-01-AC0: Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `C10-01-AC1`, build minimal fixture proving: Unknown regime menghasilkan cash. Write `test_c10_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `C10-01-AC2`, build minimal fixture proving: Perubahan future regime tidak mengubah decision. Write `test_c10_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `C10-01-AC3`, build minimal fixture proving: Member version tercatat di intent. Write `test_c10_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **C10-01-AC0**, `test_c10_01_valid_contract` — Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| C10-01-AC1 | `test_c10_01_contract_1` | Unknown regime menghasilkan cash |
| C10-01-AC2 | `test_c10_01_contract_2` | Perubahan future regime tidak mengubah decision |
| C10-01-AC3 | `test_c10_01_contract_3` | Member version tercatat di intent |
...
- Case 1: Unknown regime menghasilkan cash. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Perubahan future regime tidak mengubah decision. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Member version tercatat di intent. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `C10-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **C10-01-AC0** Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **C10-01-AC1** Unknown regime menghasilkan cash. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C10-01-AC2** Perubahan future regime tidak mengubah decision. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **C10-01-AC3** Member version tercatat di intent. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Unknown regime menghasilkan cash. Inspect fixture and actual production path.
- Attempt to disprove: Perubahan future regime tidak mengubah decision. Inspect fixture and actual production path.
- Attempt to disprove: Member version tercatat di intent. Inspect fixture and actual production path.
...
Branch: `feat/c10-01-regime-ensemble`. Commit: `feat(c10-01): regime ensemble` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/C10-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement C10-01 only: Regime ensemble.
Read AGENTS.md, docs/sprints/strategies/C10-01-regime-ensemble.md and every Required Reading path listed in it.
...
Goal: Kandidat C10 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: Deterministic trend/reversion switch using available regime and frozen members -> versioned LONG/FLAT intent, never direct orders.
