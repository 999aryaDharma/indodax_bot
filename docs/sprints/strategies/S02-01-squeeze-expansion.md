...
Recommended Branch: `feat/s02-01-squeeze-expansion`
...
Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `S02-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: BB/Keltner squeeze followed by volume expansion and no-chase cap -> versioned LONG/FLAT intent, never direct orders.
...
Given input yang memenuhi kontrak `S02-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
...
0. **S02-01-FR0:** Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
1. **S02-01-FR1:** Squeeze saja belum entry.
2. **S02-01-FR2:** Expansion tanpa volume ditolak.
3. **S02-01-FR3:** Gap di atas chase cap ditolak.
...
- `configs/strategies/S02_v1.yaml`
- `src/indodax_lab/strategies/s02.py`
- `tests/unit/lab/strategies/test_s02.py`
...
First establish S02-01-AC0: Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `S02-01-AC1`, build minimal fixture proving: Squeeze saja belum entry. Write `test_s02_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `S02-01-AC2`, build minimal fixture proving: Expansion tanpa volume ditolak. Write `test_s02_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `S02-01-AC3`, build minimal fixture proving: Gap di atas chase cap ditolak. Write `test_s02_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **S02-01-AC0**, `test_s02_01_valid_contract` — Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| S02-01-AC1 | `test_s02_01_contract_1` | Squeeze saja belum entry |
| S02-01-AC2 | `test_s02_01_contract_2` | Expansion tanpa volume ditolak |
| S02-01-AC3 | `test_s02_01_contract_3` | Gap di atas chase cap ditolak |
...
- Case 1: Squeeze saja belum entry. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Expansion tanpa volume ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Gap di atas chase cap ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `S02-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **S02-01-AC0** Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **S02-01-AC1** Squeeze saja belum entry. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S02-01-AC2** Expansion tanpa volume ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **S02-01-AC3** Gap di atas chase cap ditolak. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Squeeze saja belum entry. Inspect fixture and actual production path.
- Attempt to disprove: Expansion tanpa volume ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Gap di atas chase cap ditolak. Inspect fixture and actual production path.
...
Branch: `feat/s02-01-squeeze-expansion`. Commit: `feat(s02-01): squeeze expansion` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/S02-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: QA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement S02-01 only: Squeeze expansion.
Read AGENTS.md, docs/sprints/strategies/S02-01-squeeze-expansion.md and every Required Reading path listed in it.
...
Goal: Kandidat S02 menghasilkan intent yang dapat dibandingkan dengan baseline pada judge yang sama.
Contract: BB/Keltner squeeze followed by volume expansion and no-chase cap -> versioned LONG/FLAT intent, never direct orders.
