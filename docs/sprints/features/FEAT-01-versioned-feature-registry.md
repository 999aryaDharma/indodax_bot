...
Recommended Branch: `feat/feat-01-versioned-feature-registry`
...
Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `FEAT-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: registry YAML -> feature definitions and source hash; exact contracted names after expansion.

Direct consumers: FEAT-02, FEAT-03
...
- Kapabilitas downstream: FEAT-02, FEAT-03.
...
Given input yang memenuhi kontrak `FEAT-01` dan dependency evidence yang valid, when owning component dijalankan, then definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap.
...
0. **FEAT-01-FR0:** Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap.
1. **FEAT-01-FR1:** Nama duplikat dan bfill ditolak.
2. **FEAT-01-FR2:** Lookback kurang dari kebutuhan rumus ditolak.
3. **FEAT-01-FR3:** Perubahan konten tanpa version bump ditolak.
...
- `src/indodax_lab/features/registry.py`
- `tests/unit/lab/features/test_registry.py`
...
First establish FEAT-01-AC0: Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `FEAT-01-AC1`, build minimal fixture proving: Nama duplikat dan bfill ditolak. Write `test_feat_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `FEAT-01-AC2`, build minimal fixture proving: Lookback kurang dari kebutuhan rumus ditolak. Write `test_feat_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `FEAT-01-AC3`, build minimal fixture proving: Perubahan konten tanpa version bump ditolak. Write `test_feat_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **FEAT-01-AC0**, `test_feat_01_valid_contract` — Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| FEAT-01-AC1 | `test_feat_01_contract_1` | Nama duplikat dan bfill ditolak |
| FEAT-01-AC2 | `test_feat_01_contract_2` | Lookback kurang dari kebutuhan rumus ditolak |
| FEAT-01-AC3 | `test_feat_01_contract_3` | Perubahan konten tanpa version bump ditolak |
...
- Case 1: Nama duplikat dan bfill ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Lookback kurang dari kebutuhan rumus ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Perubahan konten tanpa version bump ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `FEAT-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **FEAT-01-AC0** Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **FEAT-01-AC1** Nama duplikat dan bfill ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-01-AC2** Lookback kurang dari kebutuhan rumus ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-01-AC3** Perubahan konten tanpa version bump ditolak. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Nama duplikat dan bfill ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Lookback kurang dari kebutuhan rumus ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Perubahan konten tanpa version bump ditolak. Inspect fixture and actual production path.
...
Branch: `feat/feat-01-versioned-feature-registry`. Commit: `feat(feat-01): versioned feature registry` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/FEAT-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: FEAT-02, FEAT-03. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement FEAT-01 only: Versioned feature registry.
Read AGENTS.md, docs/sprints/features/FEAT-01-versioned-feature-registry.md and every Required Reading path listed in it.
...
Goal: Definisi fitur Wave 1 dapat dimuat dengan identitas versi dan metadata lengkap.
Contract: registry YAML -> feature definitions and source hash; exact contracted names after expansion.
