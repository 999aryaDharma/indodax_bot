...
Recommended Branch: `feat/feat-04-immutable-feature-materialization`
...
CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `FEAT-04` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: snapshot IDs + registry hash -> features manifest, ordered columns, row_ready_at, eligibility.

Direct consumers: LABEL-01, TRAIN-01, STRAT-01, LOB-01
...
- Dependency FEAT-02 supplies: closed OHLCV -> normalized EMA RSI StochRSI MACD ATR ADX BB Donchian VWAP returns and liquidity features.
- Dependency FEAT-03 supplies: decision_ts + PIT universe + context series -> availability-safe aligned context with missing reasons.
...
- Kapabilitas downstream: LABEL-01, TRAIN-01, STRAT-01, LOB-01.
...
Given input yang memenuhi kontrak `FEAT-04` dan dependency evidence yang valid, when owning component dijalankan, then cLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh.
...
0. **FEAT-04-FR0:** CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh.
1. **FEAT-04-FR1:** Required feature null membuat row ineligible.
2. **FEAT-04-FR2:** Label atau future column tidak masuk feature schema.
3. **FEAT-04-FR3:** Dua root menghasilkan value-equivalent features dengan tolerance terdaftar.
...
- `src/indodax_lab/features/builder.py`
- `src/indodax_lab/cli/build_features.py`
- `tests/integration/lab/test_feature_materialization.py`
...
First establish FEAT-04-AC0: CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `FEAT-04-AC1`, build minimal fixture proving: Required feature null membuat row ineligible. Write `test_feat_04_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `FEAT-04-AC2`, build minimal fixture proving: Label atau future column tidak masuk feature schema. Write `test_feat_04_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `FEAT-04-AC3`, build minimal fixture proving: Dua root menghasilkan value-equivalent features dengan tolerance terdaftar. Write `test_feat_04_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **FEAT-04-AC0**, `test_feat_04_valid_contract` — CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| FEAT-04-AC1 | `test_feat_04_contract_1` | Required feature null membuat row ineligible |
| FEAT-04-AC2 | `test_feat_04_contract_2` | Label atau future column tidak masuk feature schema |
| FEAT-04-AC3 | `test_feat_04_contract_3` | Dua root menghasilkan value-equivalent features dengan tolerance terdaftar |
...
- Case 1: Required feature null membuat row ineligible. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Label atau future column tidak masuk feature schema. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Dua root menghasilkan value-equivalent features dengan tolerance terdaftar. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `FEAT-04` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **FEAT-04-AC0** CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **FEAT-04-AC1** Required feature null membuat row ineligible. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-04-AC2** Label atau future column tidak masuk feature schema. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-04-AC3** Dua root menghasilkan value-equivalent features dengan tolerance terdaftar. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Required feature null membuat row ineligible. Inspect fixture and actual production path.
- Attempt to disprove: Label atau future column tidak masuk feature schema. Inspect fixture and actual production path.
- Attempt to disprove: Dua root menghasilkan value-equivalent features dengan tolerance terdaftar. Inspect fixture and actual production path.
...
Branch: `feat/feat-04-immutable-feature-materialization`. Commit: `feat(feat-04): immutable feature materialization` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/FEAT-04-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: LABEL-01, TRAIN-01, STRAT-01, LOB-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement FEAT-04 only: Immutable feature materialization.
Read AGENTS.md, docs/sprints/features/FEAT-04-immutable-feature-materialization.md and every Required Reading path listed in it.
...
Goal: CLI membangun feature matrix dengan sample identity, warmup mask dan lineage penuh.
Contract: snapshot IDs + registry hash -> features manifest, ordered columns, row_ready_at, eligibility.
