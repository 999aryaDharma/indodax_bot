...
Recommended Branch: `feat/feat-03-as-of-market-context`
...
Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `FEAT-03` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: decision_ts + PIT universe + context series -> availability-safe aligned context with missing reasons.
...
Given input yang memenuhi kontrak `FEAT-03` dan dependency evidence yang valid, when owning component dijalankan, then fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan.
...
0. **FEAT-03-FR0:** Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan.
1. **FEAT-03-FR1:** Daily atau 4h partial bar dikecualikan.
2. **FEAT-03-FR2:** Future listing dan future cap tidak mengubah rank historis.
3. **FEAT-03-FR3:** Missing BTC history menghasilkan null bukan backfill.
...
- `src/indodax_lab/features/context.py`
- `tests/unit/lab/features/test_availability.py`
...
First establish FEAT-03-AC0: Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `FEAT-03-AC1`, build minimal fixture proving: Daily atau 4h partial bar dikecualikan. Write `test_feat_03_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `FEAT-03-AC2`, build minimal fixture proving: Future listing dan future cap tidak mengubah rank historis. Write `test_feat_03_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `FEAT-03-AC3`, build minimal fixture proving: Missing BTC history menghasilkan null bukan backfill. Write `test_feat_03_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **FEAT-03-AC0**, `test_feat_03_valid_contract` — Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| FEAT-03-AC1 | `test_feat_03_contract_1` | Daily atau 4h partial bar dikecualikan |
| FEAT-03-AC2 | `test_feat_03_contract_2` | Future listing dan future cap tidak mengubah rank historis |
| FEAT-03-AC3 | `test_feat_03_contract_3` | Missing BTC history menghasilkan null bukan backfill |
...
- Case 1: Daily atau 4h partial bar dikecualikan. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Future listing dan future cap tidak mengubah rank historis. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Missing BTC history menghasilkan null bukan backfill. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `FEAT-03` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **FEAT-03-AC0** Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **FEAT-03-AC1** Daily atau 4h partial bar dikecualikan. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-03-AC2** Future listing dan future cap tidak mengubah rank historis. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **FEAT-03-AC3** Missing BTC history menghasilkan null bukan backfill. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Daily atau 4h partial bar dikecualikan. Inspect fixture and actual production path.
- Attempt to disprove: Future listing dan future cap tidak mengubah rank historis. Inspect fixture and actual production path.
- Attempt to disprove: Missing BTC history menghasilkan null bukan backfill. Inspect fixture and actual production path.
...
Branch: `feat/feat-03-as-of-market-context`. Commit: `feat(feat-03): as-of market context` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/FEAT-03-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: FEAT-04. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement FEAT-03 only: As-of market context.
Read AGENTS.md, docs/sprints/features/FEAT-03-as-of-market-context.md and every Required Reading path listed in it.
...
Goal: Fitur BTC, rank dan breadth hanya memakai universe dan closed bars yang tersedia saat keputusan.
Contract: decision_ts + PIT universe + context series -> availability-safe aligned context with missing reasons.
