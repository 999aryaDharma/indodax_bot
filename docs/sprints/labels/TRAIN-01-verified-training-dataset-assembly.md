...
Recommended Branch: `feat/train-01-verified-training-dataset-assembly`
...
Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `TRAIN-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: dataset/feature/label/split/universe/cost IDs -> training manifest and role-restricted tables.

Direct consumers: ML-01
...
Given input yang memenuhi kontrak `TRAIN-01` dan dependency evidence yang valid, when owning component dijalankan, then materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi.
...
0. **TRAIN-01-FR0:** Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi.
1. **TRAIN-01-FR1:** Duplicate sample join ditolak.
2. **TRAIN-01-FR2:** Target kolom tidak boleh berada dalam inference feature list.
3. **TRAIN-01-FR3:** Checksum atau availability mismatch memblokir output.
...
- `src/indodax_lab/cli/build_training_dataset.py`
- `src/indodax_lab/labels/materializer.py`
...
First establish TRAIN-01-AC0: Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `TRAIN-01-AC1`, build minimal fixture proving: Duplicate sample join ditolak. Write `test_train_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `TRAIN-01-AC2`, build minimal fixture proving: Target kolom tidak boleh berada dalam inference feature list. Write `test_train_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `TRAIN-01-AC3`, build minimal fixture proving: Checksum atau availability mismatch memblokir output. Write `test_train_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **TRAIN-01-AC0**, `test_train_01_valid_contract` — Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi. Prove the valid output through public inputs, not only rejection behavior.
...
- Case 1: Duplicate sample join ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Target kolom tidak boleh berada dalam inference feature list. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Checksum atau availability mismatch memblokir output. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `TRAIN-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **TRAIN-01-AC0** Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **TRAIN-01-AC1** Duplicate sample join ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **TRAIN-01-AC2** Target kolom tidak boleh berada dalam inference feature list. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **TRAIN-01-AC3** Checksum atau availability mismatch memblokir output. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Duplicate sample join ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Target kolom tidak boleh berada dalam inference feature list. Inspect fixture and actual production path.
- Attempt to disprove: Checksum atau availability mismatch memblokir output. Inspect fixture and actual production path.
...
Branch: `feat/train-01-verified-training-dataset-assembly`. Commit: `feat(train-01): verified training dataset assembly` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/TRAIN-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: ML-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement TRAIN-01 only: Verified training dataset assembly.
Read AGENTS.md, docs/sprints/labels/TRAIN-01-verified-training-dataset-assembly.md and every Required Reading path listed in it.
...
Goal: Materializer menggabungkan fitur label dan fold hanya melalui ID yang telah diverifikasi.
Contract: dataset/feature/label/split/universe/cost IDs -> training manifest and role-restricted tables.
