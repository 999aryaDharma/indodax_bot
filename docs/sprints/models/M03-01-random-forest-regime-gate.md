...
Recommended Branch: `feat/m03-01-random-forest-regime-gate`
...
Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `M03-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Train-only regime labels -> calibrated risk classification
...
Given input yang memenuhi kontrak `M03-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
...
0. **M03-01-FR0:** Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
1. **M03-01-FR1:** Test regime tidak melatih model.
2. **M03-01-FR2:** Abstain untuk unknown class.
3. **M03-01-FR3:** Report downside dan net utility.
...
- `src/indodax_lab/models/m03.py`
- `tests/unit/lab/models/test_m03.py`
...
First establish M03-01-AC0: Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `M03-01-AC1`, build minimal fixture proving: Test regime tidak melatih model. Write `test_m03_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `M03-01-AC2`, build minimal fixture proving: Abstain untuk unknown class. Write `test_m03_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `M03-01-AC3`, build minimal fixture proving: Report downside dan net utility. Write `test_m03_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **M03-01-AC0**, `test_m03_01_valid_contract` — Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| M03-01-AC1 | `test_m03_01_contract_1` | Test regime tidak melatih model |
| M03-01-AC2 | `test_m03_01_contract_2` | Abstain untuk unknown class |
| M03-01-AC3 | `test_m03_01_contract_3` | Report downside dan net utility |
...
- Case 1: Test regime tidak melatih model. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Abstain untuk unknown class. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Report downside dan net utility. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `M03-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **M03-01-AC0** Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **M03-01-AC1** Test regime tidak melatih model. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **M03-01-AC2** Abstain untuk unknown class. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **M03-01-AC3** Report downside dan net utility. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Test regime tidak melatih model. Inspect fixture and actual production path.
- Attempt to disprove: Abstain untuk unknown class. Inspect fixture and actual production path.
- Attempt to disprove: Report downside dan net utility. Inspect fixture and actual production path.
...
Branch: `feat/m03-01-random-forest-regime-gate`. Commit: `feat(m03-01): random forest regime gate` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/M03-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement M03-01 only: Random forest regime gate.
Read AGENTS.md, docs/sprints/models/M03-01-random-forest-regime-gate.md and every Required Reading path listed in it.
...
Goal: Kandidat M03-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
