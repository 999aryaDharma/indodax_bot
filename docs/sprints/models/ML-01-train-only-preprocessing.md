...
Priority: P0 | Type: data | Domain: models | Portfolio: CORE

Implementation Owner: UNASSIGNED | Independent Reviewer: UNASSIGNED

Recommended Branch: `feat/ml-01-train-only-preprocessing`
...
Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `ML-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: train features -> imputer/scaler/pruner artifact; validation transform only.

Direct consumers: ML-02
...
- Dependency TRAIN-01 supplies: dataset/feature/label/split/universe/cost IDs -> training manifest and role-restricted tables.
...
Given input yang memenuhi kontrak `ML-01` dan dependency evidence yang valid, when owning component dijalankan, then preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test.
...
0. **ML-01-FR0:** Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test.
1. **ML-01-FR1:** Test extreme value tidak memengaruhi train median.
2. **ML-01-FR2:** Missing extra reordered feature ditolak.
3. **ML-01-FR3:** Transform tidak menjalankan fit.
...
- `src/indodax_lab/models/preprocessing.py`
- `tests/unit/lab/models/test_preprocessing.py`
...
First establish ML-01-AC0: Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `ML-01-AC1`, build minimal fixture proving: Test extreme value tidak memengaruhi train median. Write `test_ml_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `ML-01-AC2`, build minimal fixture proving: Missing extra reordered feature ditolak. Write `test_ml_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `ML-01-AC3`, build minimal fixture proving: Transform tidak menjalankan fit. Write `test_ml_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **ML-01-AC0**, `test_ml_01_valid_contract` — Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test. Prove the valid output through public inputs, not only rejection behavior.
...
- Case 1: Test extreme value tidak memengaruhi train median. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Missing extra reordered feature ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Transform tidak menjalankan fit. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `ML-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **ML-01-AC0** Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **ML-01-AC1** Test extreme value tidak memengaruhi train median. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **ML-01-AC2** Missing extra reordered feature ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **ML-01-AC3** Transform tidak menjalankan fit. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Test extreme value tidak memengaruhi train median. Inspect fixture and actual production path.
- Attempt to disprove: Missing extra reordered feature ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Transform tidak menjalankan fit. Inspect fixture and actual production path.
...
Branch: `feat/ml-01-train-only-preprocessing`. Commit: `feat(ml-01): train-only preprocessing` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/ML-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: ML-02. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement ML-01 only: Train-only preprocessing.
Read AGENTS.md, docs/sprints/models/ML-01-train-only-preprocessing.md and every Required Reading path listed in it.
...
Goal: Preprocessor menyimpan fitted statistics dan feature order tanpa mengakses test.
Contract: train features -> imputer/scaler/pruner artifact; validation transform only.
