...
Recommended Branch: `feat/ml-03-bounded-trial-search`
...
Search mencatat trial budget dan tidak memakai sealed test sebagai objective.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `ML-03` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: versioned search space + inner folds + family budget -> frozen winning recipe, all trial outcomes.

Direct consumers: M01-01, M02-01
...
- Dependency ML-02 supplies: forecast(kind=NET_RETURN/GROSS_RETURN/PROBABILITY) + payoff/cost basis -> abstain or intent; net costs applied once.
...
- Kapabilitas downstream: M01-01, M02-01.
...
Given input yang memenuhi kontrak `ML-03` dan dependency evidence yang valid, when owning component dijalankan, then search mencatat trial budget dan tidak memakai sealed test sebagai objective.
...
0. **ML-03-FR0:** Search mencatat trial budget dan tidak memakai sealed test sebagai objective.
1. **ML-03-FR1:** Trial gagal tetap menghabiskan budget.
2. **ML-03-FR2:** Resume config mismatch ditolak.
3. **ML-03-FR3:** Maksimum 30 trial dan satu near-miss revision per model.
...
- `src/indodax_lab/models/tuning.py`
- `tests/unit/lab/models/test_tuning_budget.py`
...
First establish ML-03-AC0: Search mencatat trial budget dan tidak memakai sealed test sebagai objective. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `ML-03-AC1`, build minimal fixture proving: Trial gagal tetap menghabiskan budget. Write `test_ml_03_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `ML-03-AC2`, build minimal fixture proving: Resume config mismatch ditolak. Write `test_ml_03_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `ML-03-AC3`, build minimal fixture proving: Maksimum 30 trial dan satu near-miss revision per model. Write `test_ml_03_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **ML-03-AC0**, `test_ml_03_valid_contract` — Search mencatat trial budget dan tidak memakai sealed test sebagai objective. Prove the valid output through public inputs, not only rejection behavior.
...
- Case 1: Trial gagal tetap menghabiskan budget. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Resume config mismatch ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Maksimum 30 trial dan satu near-miss revision per model. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `ML-03` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **ML-03-AC0** Search mencatat trial budget dan tidak memakai sealed test sebagai objective. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **ML-03-AC1** Trial gagal tetap menghabiskan budget. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **ML-03-AC2** Resume config mismatch ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **ML-03-AC3** Maksimum 30 trial dan satu near-miss revision per model. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Trial gagal tetap menghabiskan budget. Inspect fixture and actual production path.
- Attempt to disprove: Resume config mismatch ditolak. Inspect fixture and actual production path.
- Attempt to disprove: Maksimum 30 trial dan satu near-miss revision per model. Inspect fixture and actual production path.
...
Branch: `feat/ml-03-bounded-trial-search`. Commit: `feat(ml-03): bounded trial search` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/ML-03-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: M01-01, M02-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement ML-03 only: Bounded trial search.
Read AGENTS.md, docs/sprints/models/ML-03-bounded-trial-search.md and every Required Reading path listed in it.
...
Goal: Search mencatat trial budget dan tidak memakai sealed test sebagai objective.
Contract: versioned search space + inner folds + family budget -> frozen winning recipe, all trial outcomes.
