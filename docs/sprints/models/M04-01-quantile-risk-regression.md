...
Recommended Branch: `feat/m04-01-quantile-risk-regression`
...
Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `M04-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Registered return volatility or tail quantile target -> interval forecast
...
Given input yang memenuhi kontrak `M04-01` dan dependency evidence yang valid, when owning component dijalankan, then kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
...
0. **M04-01-FR0:** Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
1. **M04-01-FR1:** Quantile crossing ditangani eksplisit.
2. **M04-01-FR2:** Coverage out-of-sample tercatat.
3. **M04-01-FR3:** Tail target tidak masuk input.
...
- `src/indodax_lab/models/m04.py`
- `tests/unit/lab/models/test_m04.py`
...
First establish M04-01-AC0: Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `M04-01-AC1`, build minimal fixture proving: Quantile crossing ditangani eksplisit. Write `test_m04_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `M04-01-AC2`, build minimal fixture proving: Coverage out-of-sample tercatat. Write `test_m04_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `M04-01-AC3`, build minimal fixture proving: Tail target tidak masuk input. Write `test_m04_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **M04-01-AC0**, `test_m04_01_valid_contract` — Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| M04-01-AC1 | `test_m04_01_contract_1` | Quantile crossing ditangani eksplisit |
| M04-01-AC2 | `test_m04_01_contract_2` | Coverage out-of-sample tercatat |
| M04-01-AC3 | `test_m04_01_contract_3` | Tail target tidak masuk input |
...
- Case 1: Quantile crossing ditangani eksplisit. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Coverage out-of-sample tercatat. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Tail target tidak masuk input. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `M04-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **M04-01-AC0** Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **M04-01-AC1** Quantile crossing ditangani eksplisit. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **M04-01-AC2** Coverage out-of-sample tercatat. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **M04-01-AC3** Tail target tidak masuk input. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Quantile crossing ditangani eksplisit. Inspect fixture and actual production path.
- Attempt to disprove: Coverage out-of-sample tercatat. Inspect fixture and actual production path.
- Attempt to disprove: Tail target tidak masuk input. Inspect fixture and actual production path.
...
Branch: `feat/m04-01-quantile-risk-regression`. Commit: `feat(m04-01): quantile risk regression` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/M04-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement M04-01 only: Quantile risk regression.
Read AGENTS.md, docs/sprints/models/M04-01-quantile-risk-regression.md and every Required Reading path listed in it.
...
Goal: Kandidat M04-01 menambah hipotesis risk/forecast yang terukur setelah Wave 1.
