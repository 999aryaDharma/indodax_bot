...
Setiap percobaan termasuk gagal tersimpan dengan konfigurasi dan ancestry yang dapat diaudit.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `EVAL-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: run_id + parent + git SHA + environment/data/config/cost/execution hashes -> immutable run record.

Direct consumers: EVAL-02, ML-03, JOB-01, OPS-03
...
- Kapabilitas downstream: EVAL-02, ML-03, JOB-01, OPS-03.
...
Given input yang memenuhi kontrak `EVAL-01` dan dependency evidence yang valid, when owning component dijalankan, then setiap percobaan termasuk gagal tersimpan dengan konfigurasi dan ancestry yang dapat diaudit.
...
0. **EVAL-01-FR0:** Setiap percobaan termasuk gagal tersimpan dengan konfigurasi dan ancestry yang dapat diaudit.
1. **EVAL-01-FR1:** Dirty worktree tidak memenuhi promotable run.
2. **EVAL-01-FR2:** Failed trial ikut trial count.
3. **EVAL-01-FR3:** Duplicate run key tidak menimpa hasil berbeda.
...
First establish EVAL-01-AC0: Setiap percobaan termasuk gagal tersimpan dengan konfigurasi dan ancestry yang dapat diaudit. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `EVAL-01-AC1`, build minimal fixture proving: Dirty worktree tidak memenuhi promotable run. Write `test_eval_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `EVAL-01-AC2`, build minimal fixture proving: Failed trial ikut trial count. Write `test_eval_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `EVAL-01-AC3`, build minimal fixture proving: Duplicate run key tidak menimpa hasil berbeda. Write `test_eval_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
- Case 1: Dirty worktree tidak memenuhi promotable run. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Failed trial ikut trial count. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Duplicate run key tidak menimpa hasil berbeda. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `EVAL-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- Attempt to disprove: Dirty worktree tidak memenuhi promotable run. Inspect fixture and actual production path.
- Attempt to disprove: Failed trial ikut trial count. Inspect fixture and actual production path.
- Attempt to disprove: Duplicate run key tidak menimpa hasil berbeda. Inspect fixture and actual production path.
...
Branch: `feat/eval-01-immutable-experiment-registry`. Commit: `feat(eval-01): immutable experiment registry` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/EVAL-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: EVAL-02, ML-03, JOB-01, OPS-03. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement EVAL-01 only: Immutable experiment registry.
Read AGENTS.md, docs/sprints/evaluation/EVAL-01-immutable-experiment-registry.md and every Required Reading path listed in it.
...
Goal: Setiap percobaan termasuk gagal tersimpan dengan konfigurasi dan ancestry yang dapat diaudit.
Contract: run_id + parent + git SHA + environment/data/config/cost/execution hashes -> immutable run record.
