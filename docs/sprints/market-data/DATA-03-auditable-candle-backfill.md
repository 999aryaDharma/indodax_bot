...
Recommended Branch: `feat/data-03-auditable-candle-backfill`

Requirements: FR-02 | Legacy tasks: 9
...
Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `DATA-03` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: run_backfill(pair, interval, start, end, transport) -> wire, bronze, manifest, checkpoint; [start,end).

Direct consumers: DATA-04
...
- Kapabilitas downstream: DATA-04.
...
Given input yang memenuhi kontrak `DATA-03` dan dependency evidence yang valid, when owning component dijalankan, then backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable.
...
0. **DATA-03-FR0:** Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable.
1. **DATA-03-FR1:** Dry-run tidak menulis atau mengakses jaringan.
2. **DATA-03-FR2:** Malformed row masuk reject list tanpa harga nol.
3. **DATA-03-FR3:** Checkpoint diverifikasi bersama wire metadata dan checksum.
...
First establish DATA-03-AC0: Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `DATA-03-AC1`, build minimal fixture proving: Dry-run tidak menulis atau mengakses jaringan. Write `test_data_03_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `DATA-03-AC2`, build minimal fixture proving: Malformed row masuk reject list tanpa harga nol. Write `test_data_03_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `DATA-03-AC3`, build minimal fixture proving: Checkpoint diverifikasi bersama wire metadata dan checksum. Write `test_data_03_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **DATA-03-AC0**, `test_data_03_valid_contract` — Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| DATA-03-AC1 | `test_data_03_contract_1` | Dry-run tidak menulis atau mengakses jaringan |
| DATA-03-AC2 | `test_data_03_contract_2` | Malformed row masuk reject list tanpa harga nol |
| DATA-03-AC3 | `test_data_03_contract_3` | Checkpoint diverifikasi bersama wire metadata dan checksum |
...
- Case 1: Dry-run tidak menulis atau mengakses jaringan. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Malformed row masuk reject list tanpa harga nol. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Checkpoint diverifikasi bersama wire metadata dan checksum. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `DATA-03` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **DATA-03-AC0** Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **DATA-03-AC1** Dry-run tidak menulis atau mengakses jaringan. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-03-AC2** Malformed row masuk reject list tanpa harga nol. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-03-AC3** Checkpoint diverifikasi bersama wire metadata dan checksum. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Dry-run tidak menulis atau mengakses jaringan. Inspect fixture and actual production path.
- Attempt to disprove: Malformed row masuk reject list tanpa harga nol. Inspect fixture and actual production path.
- Attempt to disprove: Checkpoint diverifikasi bersama wire metadata dan checksum. Inspect fixture and actual production path.
...
Branch: `feat/data-03-auditable-candle-backfill`. Commit: `fix(data-03): auditable candle backfill` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/DATA-03-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: DATA-04. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement DATA-03 only: Auditable candle backfill.
Read AGENTS.md, docs/sprints/market-data/DATA-03-auditable-candle-backfill.md and every Required Reading path listed in it.
...
Goal: Backfill menyimpan wire asli sebelum parsing dan resume hanya dari window yang durable.
Contract: run_backfill(pair, interval, start, end, transport) -> wire, bronze, manifest, checkpoint; [start,end).
