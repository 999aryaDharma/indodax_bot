...
Priority: P0 | Type: data | Domain: market-data | Portfolio: CORE

Implementation Owner: UNASSIGNED | Independent Reviewer: UNASSIGNED

Recommended Branch: `feat/data-04-snapshot-quality-decisions`

Requirements: FR-02 | Legacy tasks: 10
...
Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `DATA-04` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: validate_snapshot(snapshot, coverage, as_of) -> canonical quality report, CLI exit 0/3/4.

Direct consumers: UNIV-01, BAR-01
...
- Dependency DATA-03 supplies: run_backfill(pair, interval, start, end, transport) -> wire, bronze, manifest, checkpoint; [start,end).
...
- Kapabilitas downstream: UNIV-01, BAR-01.
...
Given input yang memenuhi kontrak `DATA-04` dan dependency evidence yang valid, when owning component dijalankan, then snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible.
...
0. **DATA-04-FR0:** Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible.
1. **DATA-04-FR1:** FAIL QUARANTINED upstream tidak menjadi PASS.
2. **DATA-04-FR2:** Coverage invalid exit 4 dan corrupt content exit 3.
3. **DATA-04-FR3:** Path traversal dan symlink loop ditolak terkendali.
...
- `src/indodax_lab/data/sentry.py`
- `src/indodax_lab/data/quality.py`
- `src/indodax_lab/cli/validate_snapshot.py`
- `tests/integration/lab/test_snapshot_validation.py`
...
First establish DATA-04-AC0: Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `DATA-04-AC1`, build minimal fixture proving: FAIL QUARANTINED upstream tidak menjadi PASS. Write `test_data_04_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `DATA-04-AC2`, build minimal fixture proving: Coverage invalid exit 4 dan corrupt content exit 3. Write `test_data_04_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `DATA-04-AC3`, build minimal fixture proving: Path traversal dan symlink loop ditolak terkendali. Write `test_data_04_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **DATA-04-AC0**, `test_data_04_valid_contract` — Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| DATA-04-AC1 | `test_data_04_contract_1` | FAIL QUARANTINED upstream tidak menjadi PASS |
| DATA-04-AC2 | `test_data_04_contract_2` | Coverage invalid exit 4 dan corrupt content exit 3 |
| DATA-04-AC3 | `test_data_04_contract_3` | Path traversal dan symlink loop ditolak terkendali |
...
- Case 1: FAIL QUARANTINED upstream tidak menjadi PASS. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Coverage invalid exit 4 dan corrupt content exit 3. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Path traversal dan symlink loop ditolak terkendali. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `DATA-04` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **DATA-04-AC0** Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **DATA-04-AC1** FAIL QUARANTINED upstream tidak menjadi PASS. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-04-AC2** Coverage invalid exit 4 dan corrupt content exit 3. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-04-AC3** Path traversal dan symlink loop ditolak terkendali. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: FAIL QUARANTINED upstream tidak menjadi PASS. Inspect fixture and actual production path.
- Attempt to disprove: Coverage invalid exit 4 dan corrupt content exit 3. Inspect fixture and actual production path.
- Attempt to disprove: Path traversal dan symlink loop ditolak terkendali. Inspect fixture and actual production path.
...
Branch: `feat/data-04-snapshot-quality-decisions`. Commit: `fix(data-04): snapshot quality decisions` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/DATA-04-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: UNIV-01, BAR-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement DATA-04 only: Snapshot quality decisions.
Read AGENTS.md, docs/sprints/market-data/DATA-04-snapshot-quality-decisions.md and every Required Reading path listed in it.
...
Goal: Snapshot rusak menghasilkan quality finding dan tidak dapat menjadi input eligible.
Contract: validate_snapshot(snapshot, coverage, as_of) -> canonical quality report, CLI exit 0/3/4.
