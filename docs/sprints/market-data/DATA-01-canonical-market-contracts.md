...
Recommended Branch: `feat/data-01-canonical-market-contracts`

Requirements: FR-02 | Legacy tasks: 7
...
Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `DATA-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: CanonicalPair, Candle, TradeEvent + path roots -> validated immutable records.

Direct consumers: DATA-02, COST-01
...
- Kapabilitas downstream: DATA-02, COST-01.
...
Given input yang memenuhi kontrak `DATA-01` dan dependency evidence yang valid, when owning component dijalankan, then collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat.
...
0. **DATA-01-FR0:** Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat.
1. **DATA-01-FR1:** schema_version wajib.
2. **DATA-01-FR2:** Interval tidak dikenal ditolak.
3. **DATA-01-FR3:** ingested_at sebelum event_ts memerlukan anomaly flag.
...
- `src/indodax_lab/contracts`
- `src/indodax_lab/paths.py`
- `tests/unit/lab/test_contracts.py`
...
First establish DATA-01-AC0: Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `DATA-01-AC1`, build minimal fixture proving: schema_version wajib. Write `test_data_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `DATA-01-AC2`, build minimal fixture proving: Interval tidak dikenal ditolak. Write `test_data_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `DATA-01-AC3`, build minimal fixture proving: ingested_at sebelum event_ts memerlukan anomaly flag. Write `test_data_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **DATA-01-AC0**, `test_data_01_valid_contract` — Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| DATA-01-AC1 | `test_data_01_contract_1` | schema_version wajib |
| DATA-01-AC2 | `test_data_01_contract_2` | Interval tidak dikenal ditolak |
| DATA-01-AC3 | `test_data_01_contract_3` | ingested_at sebelum event_ts memerlukan anomaly flag |
...
- Case 1: schema_version wajib. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Interval tidak dikenal ditolak. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: ingested_at sebelum event_ts memerlukan anomaly flag. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `DATA-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **DATA-01-AC0** Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **DATA-01-AC1** schema_version wajib. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-01-AC2** Interval tidak dikenal ditolak. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **DATA-01-AC3** ingested_at sebelum event_ts memerlukan anomaly flag. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: schema_version wajib. Inspect fixture and actual production path.
- Attempt to disprove: Interval tidak dikenal ditolak. Inspect fixture and actual production path.
- Attempt to disprove: ingested_at sebelum event_ts memerlukan anomaly flag. Inspect fixture and actual production path.
...
Branch: `feat/data-01-canonical-market-contracts`. Commit: `fix(data-01): canonical market contracts` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/DATA-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: DATA-02, COST-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement DATA-01 only: Canonical market contracts.
Read AGENTS.md, docs/sprints/market-data/DATA-01-canonical-market-contracts.md and every Required Reading path listed in it.
...
Goal: Collector dan lab berbagi identity pair, UTC, Decimal dan version lineage yang ketat.
Contract: CanonicalPair, Candle, TradeEvent + path roots -> validated immutable records.
