...
Priority: P1 | Type: research | Domain: deep-learning | Portfolio: EXTENSION

Implementation Owner: UNASSIGNED | Independent Reviewer: UNASSIGNED

Recommended Branch: `feat/d01-01-tabular-mlp-baseline`
...
MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `D01-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: same feature rows + <=12 configs -> nonlinear baseline forecast via common mapper.

Direct consumers: D02-01, G01-01, F01-02, L01-01
...
- Kapabilitas downstream: D02-01, G01-01, F01-02, L01-01.
...
Given input yang memenuhi kontrak `D01-01` dan dependency evidence yang valid, when owning component dijalankan, then mLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02.
...
0. **D01-01-FR0:** MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02.
1. **D01-01-FR1:** Same sample comparator dijaga.
2. **D01-01-FR2:** Tiga finalist seed tidak cherry-pick.
3. **D01-01-FR3:** Training interrupted dapat resume pada best checkpoint.
...
- `src/indodax_lab/models/dl/d01_mlp.py`
- `tests/integration/lab/test_d01_training_smoke.py`
...
First establish D01-01-AC0: MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `D01-01-AC1`, build minimal fixture proving: Same sample comparator dijaga. Write `test_d01_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `D01-01-AC2`, build minimal fixture proving: Tiga finalist seed tidak cherry-pick. Write `test_d01_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `D01-01-AC3`, build minimal fixture proving: Training interrupted dapat resume pada best checkpoint. Write `test_d01_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **D01-01-AC0**, `test_d01_01_valid_contract` — MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| D01-01-AC1 | `test_d01_01_contract_1` | Same sample comparator dijaga |
| D01-01-AC2 | `test_d01_01_contract_2` | Tiga finalist seed tidak cherry-pick |
...
- Case 1: Same sample comparator dijaga. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Tiga finalist seed tidak cherry-pick. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Training interrupted dapat resume pada best checkpoint. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `D01-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **D01-01-AC0** MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **D01-01-AC1** Same sample comparator dijaga. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D01-01-AC2** Tiga finalist seed tidak cherry-pick. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D01-01-AC3** Training interrupted dapat resume pada best checkpoint. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Same sample comparator dijaga. Inspect fixture and actual production path.
- Attempt to disprove: Tiga finalist seed tidak cherry-pick. Inspect fixture and actual production path.
- Attempt to disprove: Training interrupted dapat resume pada best checkpoint. Inspect fixture and actual production path.
...
Branch: `feat/d01-01-tabular-mlp-baseline`. Commit: `feat(d01-01): tabular mlp baseline` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/D01-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: D02-01, G01-01, F01-02, L01-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement D01-01 only: Tabular MLP baseline.
Read AGENTS.md, docs/sprints/deep-learning/D01-01-tabular-mlp-baseline.md and every Required Reading path listed in it.
...
Goal: MLP menguji manfaat nonlinearitas dengan fitur tabular yang sama seperti M01/M02.
Contract: same feature rows + <=12 configs -> nonlinear baseline forecast via common mapper.
