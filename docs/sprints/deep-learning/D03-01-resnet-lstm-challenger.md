...
Recommended Branch: `feat/d03-01-resnet-lstm-challenger`
...
Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `D03-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Residual temporal blocks + recurrent head -> registered triple-barrier target
...
- Dependency D02-01 supplies: Causal dilated convolutions + mask-safe pooling -> multi-horizon forecast
...
Given input yang memenuhi kontrak `D03-01` dan dependency evidence yang valid, when owning component dijalankan, then eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama.
...
0. **D03-01-FR0:** Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama.
1. **D03-01-FR1:** No bidirectional future leakage.
2. **D03-01-FR2:** Mask menjaga padded sequence.
3. **D03-01-FR3:** Common evaluation cost dan folds digunakan.
...
- `src/indodax_lab/models/dl/d03_resnet_lstm.py`
- `tests/unit/lab/models/test_d03_01.py`
...
First establish D03-01-AC0: Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `D03-01-AC1`, build minimal fixture proving: No bidirectional future leakage. Write `test_d03_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `D03-01-AC2`, build minimal fixture proving: Mask menjaga padded sequence. Write `test_d03_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `D03-01-AC3`, build minimal fixture proving: Common evaluation cost dan folds digunakan. Write `test_d03_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **D03-01-AC0**, `test_d03_01_valid_contract` — Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| D03-01-AC1 | `test_d03_01_contract_1` | No bidirectional future leakage |
| D03-01-AC2 | `test_d03_01_contract_2` | Mask menjaga padded sequence |
| D03-01-AC3 | `test_d03_01_contract_3` | Common evaluation cost dan folds digunakan |
...
- Case 1: No bidirectional future leakage. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Mask menjaga padded sequence. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Common evaluation cost dan folds digunakan. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `D03-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **D03-01-AC0** Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **D03-01-AC1** No bidirectional future leakage. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D03-01-AC2** Mask menjaga padded sequence. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D03-01-AC3** Common evaluation cost dan folds digunakan. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: No bidirectional future leakage. Inspect fixture and actual production path.
- Attempt to disprove: Mask menjaga padded sequence. Inspect fixture and actual production path.
- Attempt to disprove: Common evaluation cost dan folds digunakan. Inspect fixture and actual production path.
...
Branch: `feat/d03-01-resnet-lstm-challenger`. Commit: `feat(d03-01): resnet lstm challenger` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/D03-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement D03-01 only: ResNet LSTM challenger.
Read AGENTS.md, docs/sprints/deep-learning/D03-01-resnet-lstm-challenger.md and every Required Reading path listed in it.
...
Goal: Eksperimen D03-01 menghasilkan forecast yang tunduk pada evaluator bersama.
Contract: Residual temporal blocks + recurrent head -> registered triple-barrier target
