...
Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `D04-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: Variates as tokens with available input window -> panel forecast
...
Given input yang memenuhi kontrak `D04-01` dan dependency evidence yang valid, when owning component dijalankan, then eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama.
...
0. **D04-01-FR0:** Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama.
1. **D04-01-FR1:** Shape time-feature tidak tertukar.
2. **D04-01-FR2:** Missing variate masking diuji.
3. **D04-01-FR3:** Tidak memakai future universe.
...
- `src/indodax_lab/models/dl/d04_itransformer.py`
- `tests/unit/lab/models/test_d04_01.py`
...
First establish D04-01-AC0: Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `D04-01-AC1`, build minimal fixture proving: Shape time-feature tidak tertukar. Write `test_d04_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `D04-01-AC2`, build minimal fixture proving: Missing variate masking diuji. Write `test_d04_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `D04-01-AC3`, build minimal fixture proving: Tidak memakai future universe. Write `test_d04_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **D04-01-AC0**, `test_d04_01_valid_contract` — Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| D04-01-AC1 | `test_d04_01_contract_1` | Shape time-feature tidak tertukar |
| D04-01-AC2 | `test_d04_01_contract_2` | Missing variate masking diuji |
| D04-01-AC3 | `test_d04_01_contract_3` | Tidak memakai future universe |
...
- Case 1: Shape time-feature tidak tertukar. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Missing variate masking diuji. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Tidak memakai future universe. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `D04-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **D04-01-AC0** Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **D04-01-AC1** Shape time-feature tidak tertukar. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D04-01-AC2** Missing variate masking diuji. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **D04-01-AC3** Tidak memakai future universe. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Shape time-feature tidak tertukar. Inspect fixture and actual production path.
- Attempt to disprove: Missing variate masking diuji. Inspect fixture and actual production path.
- Attempt to disprove: Tidak memakai future universe. Inspect fixture and actual production path.
...
Branch: `feat/d04-01-compact-itransformer-challenger`. Commit: `feat(d04-01): compact itransformer challenger` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/D04-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: none required. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement D04-01 only: Compact iTransformer challenger.
Read AGENTS.md, docs/sprints/deep-learning/D04-01-compact-itransformer-challenger.md and every Required Reading path listed in it.
...
Goal: Eksperimen D04-01 menghasilkan forecast yang tunduk pada evaluator bersama.
Contract: Variates as tokens with available input window -> panel forecast
