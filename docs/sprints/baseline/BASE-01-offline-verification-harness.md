...
Recommended Branch: `feat/base-01-offline-verification-harness`

Requirements: FR-01 | Legacy tasks: 1
...
CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata.

## Why This Sprint Exists

Tanpa kapabilitas ini, kontrak `BASE-01` belum dapat dibuktikan dan downstream tidak boleh mengasumsikan hasilnya tersedia. Nilai spesifiknya: pytest + injected transports -> reproducible result; no credentials required.

Direct consumers: BASE-02, BASE-03, BASE-04, DATA-01
...
- Kapabilitas downstream: BASE-02, BASE-03, BASE-04, DATA-01.
...
Given input yang memenuhi kontrak `BASE-01` dan dependency evidence yang valid, when owning component dijalankan, then cI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata.
...
0. **BASE-01-FR0:** CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata.
1. **BASE-01-FR1:** Transport nyata dipanggil -> test gagal.
2. **BASE-01-FR2:** Import modul tidak mengeksekusi scheduler.
3. **BASE-01-FR3:** Fixture deterministik menghasilkan hasil sama.
...
First establish BASE-01-AC0: CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata. Use a minimal valid fixture, independent expected output, and a failing assertion before implementation.

1. Inspect dependency handoffs and actual module paths; confirm one owner and independent reviewer. Do not mark fresh code complete from historical evidence.
2. For `BASE-01-AC1`, build minimal fixture proving: Transport nyata dipanggil -> test gagal. Write `test_base_01_contract_1` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
3. For `BASE-01-AC2`, build minimal fixture proving: Import modul tidak mengeksekusi scheduler. Write `test_base_01_contract_2` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
4. For `BASE-01-AC3`, build minimal fixture proving: Fixture deterministik menghasilkan hasil sama. Write `test_base_01_contract_3` or a clearly mapped existing test; observe targeted RED (wrong behavior, not missing test dependency), implement only that contract, then prove GREEN.
...
Positive contract: **BASE-01-AC0**, `test_base_01_valid_contract` — CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata. Prove the valid output through public inputs, not only rejection behavior.

| Acceptance | Planned test identity / map to existing equivalent | Assertion |
|---|---|---|
| BASE-01-AC1 | `test_base_01_contract_1` | Transport nyata dipanggil -> test gagal |
| BASE-01-AC2 | `test_base_01_contract_2` | Import modul tidak mengeksekusi scheduler |
| BASE-01-AC3 | `test_base_01_contract_3` | Fixture deterministik menghasilkan hasil sama |
...
- Case 1: Transport nyata dipanggil -> test gagal. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 2: Import modul tidak mengeksekusi scheduler. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
- Case 3: Fixture deterministik menghasilkan hasil sama. Expected behavior is this assertion; never fall through to a successful artifact on rejection.
...
Emit `BASE-01` scope, input identities, output/run identity, config/policy version and reason code. For each listed guard, tests must assert the diagnostic identifies what was rejected without logging private payloads. Record elapsed/resource observations only outside content-addressed identities.
...
- [ ] **BASE-01-AC0** CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata. Evidence: valid fixture through the public interface, with expected output independent of implementation.
- [ ] **BASE-01-AC1** Transport nyata dipanggil -> test gagal. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **BASE-01-AC2** Import modul tidak mengeksekusi scheduler. Evidence: mapped test, exact command/exit and target SHA.
- [ ] **BASE-01-AC3** Fixture deterministik menghasilkan hasil sama. Evidence: mapped test, exact command/exit and target SHA.
...
- Attempt to disprove: Transport nyata dipanggil -> test gagal. Inspect fixture and actual production path.
- Attempt to disprove: Import modul tidak mengeksekusi scheduler. Inspect fixture and actual production path.
- Attempt to disprove: Fixture deterministik menghasilkan hasil sama. Inspect fixture and actual production path.
...
Branch: `feat/base-01-offline-verification-harness`. Commit: `fix(base-01): offline verification harness` for future implementation. Documentation changes use `docs(...)`. Never stage original Task15 WIP wholesale.

## Handoff Requirements

Create `docs/sprints/handoffs/BASE-01-HANDOFF.md` from template. Record owner/reviewer identities, branch, code SHA, files, contracts, migration, RED/GREEN commands, exact test results, AC mappings, deviations and known risks. Specify next eligible consumers: BASE-02, BASE-03, BASE-04, DATA-01. A code commit and later evidence commit must state their relationship explicitly.

## Ready-to-Run Implementation Prompt

```text
Implement BASE-01 only: Offline verification harness.
Read AGENTS.md, docs/sprints/baseline/BASE-01-offline-verification-harness.md and every Required Reading path listed in it.
...
Goal: CI membuktikan perilaku bot dengan fixture tanpa HTTP atau Telegram nyata.
Contract: pytest + injected transports -> reproducible result; no credentials required.
