# RW6-01 — Separate shared-capital Portfolio Shadow Implementation Plan

> **For agentic workers:** LUNA uses superpowers:executing-plans task-by-task; independent reviewer is required.

**Goal:** Intentional multi-candidate capital sharing through one common ledger/risk authority.
...
Recommended Branch: `feat/rw6-01-separate-shared-capital-portfolio-shadow`
...
Intentional multi-candidate capital sharing through one common ledger/risk authority.

## Why This Sprint Exists

Purpose: Separate shared-capital Portfolio Shadow. Gap: Intentional multi-candidate capital sharing through one common ledger/risk authority.
...
SharedCapitalLedger allocates cash outside common financial runtime.
...
- Collect same-event proposals before deterministic arbitration

- Apply aggregate concentration/risk and common OMS/accounting

- Attribute fills by candidate without duplicating shared equity

- Compare shared result with isolated agents using disclosed capital basis
...
0. **RW6-01-FR0:** Simultaneous intents cannot overspend shared cash.

1. **RW6-01-FR1:** Same-pair conflicts resolve by declared stable policy.

2. **RW6-01-FR2:** Reordered callback delivery yields identical allocation.

3. **RW6-01-FR3:** Shared run cannot mutate any isolated agent.

4. **RW6-01-FR4:** Attributed totals reconcile once to shared ledger.
...
- `src/indodax_lab/paper/portfolio_shadow.py`

- `tests/integration/lab/test_portfolio_shadow_contention.py`
...
Deprecate SharedCapitalLedger for new qualification; legacy checkpoint import is explicit and never assumed fee-complete.
...
- [ ] Step 1: Resolve selected immutable candidates into one distinct namespace.

- [ ] Step 2: Collect same-event proposals before deterministic arbitration.

- [ ] Step 3: Apply aggregate concentration/risk and common OMS/accounting.

- [ ] Step 4: Attribute fills by candidate without duplicating shared equity.

- [ ] Step 5: Compare shared result with isolated agents using disclosed capital basis.
...
- [ ] Run `python -m pytest tests/integration/lab/test_portfolio_shadow_contention.py -q`; expected all named behaviors pass. Shared contract/accounting/recovery changes additionally run `python -m pytest -q` with required dependencies; required skips block acceptance.

- [ ] Run `git diff --check`, record exact environment/command/exit/SHA, commit scoped files, and submit independent review.

## Required Tests

Planned test paths: `tests/integration/lab/test_portfolio_shadow_contention.py`.

| Acceptance | Planned test identity | Required assertion |

|---|---|---|

| RW6-01-AC0 | `test_rw6_01_0` | Simultaneous intents cannot overspend shared cash |

| RW6-01-AC1 | `test_rw6_01_1` | Same-pair conflicts resolve by declared stable policy |

| RW6-01-AC2 | `test_rw6_01_2` | Reordered callback delivery yields identical allocation |

| RW6-01-AC3 | `test_rw6_01_3` | Shared run cannot mutate any isolated agent |

| RW6-01-AC4 | `test_rw6_01_4` | Attributed totals reconcile once to shared ledger |

Focused command: `python -m pytest tests/integration/lab/test_portfolio_shadow_contention.py -q`. Tests must exercise the public service boundary, not only fabricated ID equality. No real network/Telegram/DB.

## Failure / Edge Cases

- Same-pair conflicts resolve by declared stable policy. Rejection preserves prior committed evidence and emits a specific reason.

- Reordered callback delivery yields identical allocation. Rejection preserves prior committed evidence and emits a specific reason.

- Shared run cannot mutate any isolated agent. Rejection preserves prior committed evidence and emits a specific reason.

- Attributed totals reconcile once to shared ledger. Rejection preserves prior committed evidence and emits a specific reason.
...
Emit task/service reason codes with candidate/artifact/event/request IDs, namespace and policy/schema versions. RW6-01 acceptance evidence records exact code SHA and command exit. Log durations outside semantic hashes; never private payloads.
...
- [ ] **RW6-01-AC1** Same-pair conflicts resolve by declared stable policy. Evidence: named test on exact committed SHA.

- [ ] **RW6-01-AC2** Reordered callback delivery yields identical allocation. Evidence: named test on exact committed SHA.

- [ ] **RW6-01-AC3** Shared run cannot mutate any isolated agent. Evidence: named test on exact committed SHA.

- [ ] **RW6-01-AC4** Attributed totals reconcile once to shared ledger. Evidence: named test on exact committed SHA.
...
Use `feat/rw6-01-separate-shared-capital-portfolio-shadow` in an isolated worktree. Stage only listed/recorded scoped files; no broad staging. Commit message: `feat(rw6-01): separate shared-capital portfolio shadow`. No merge/push/deploy.

## Handoff Requirements

Write `docs/sprints/handoffs/RW6-01-HANDOFF.md`: owner/reviewer, base/code/evidence SHAs, actual paths, acceptance command/results, migration, deviations and pending external gates. Preserve review rounds. Completion does not follow automatically from a passing local unit test.
...
SharedCapitalLedger allocates cash outside common financial runtime. Shared-runtime/frozen authority contracts apply.
...
`src/indodax_lab/paper/portfolio.py`, `src/indodax_lab/paper/portfolio_shadow.py`, `tests/integration/lab/test_portfolio_shadow_contention.py`

### TESTS

`python -m pytest tests/integration/lab/test_portfolio_shadow_contention.py -q` and full-suite gate when required. Every AC above is mandatory.
...
Scoped committed implementation and `RW6-01-HANDOFF.md`, submitted at REVIEW, not self-approved DONE.
