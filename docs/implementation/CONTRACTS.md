...
The scheduler commits an internal `CanonicalMarketEvent` of kind `EVALUATION_TRIGGER` to the feed journal for entry/model evaluation. Its event ID derives from versioned trigger policy, immutable input/deployment scope and trigger key (UTC slot or source-event/feature-change identity); its immutable observation binds the selected as-of source cursor, market refs and FeatureSnapshot refs. Same scheduling key cannot bind different inputs. Distinct slots over the same market snapshot are distinct trigger events. Provider sequence/offset fields are null; this is a runtime-derived event, not an exchange channel. `CandidateRuntime.evaluate`, intent-ID derivation and `ExecutionStateStore.prepare_event` use the trigger event ID and journal sequence. Raw market events update marks/exits once; trigger events do not repeat those state updates. Already DECIDED triggers resume existing outbox decisions; unscheduled missed slots never become backdated live evidence. Full deadline, dedup and recovery rules are in [trigger policies](../production/research-workbench/SHARED-MARKET-RUNTIME.md#declarative-triggers).
