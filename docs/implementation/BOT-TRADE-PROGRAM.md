...
| Reporting target/window | 5% net monthly after trading costs; server operating costs excluded; no forced trading or shorter qualification |
...
- Manual resume is required after close-out and still must pass peak-equity-floor, risk, release, health and reconciliation checks; a resume command alone cannot override an unresolved floor breach. Controlled close-out is a portfolio-risk action, distinct from candidate replacement draining and from operational HALT. HALT continues to prohibit writes when its gates fail; eligible close-out work waits durably.
