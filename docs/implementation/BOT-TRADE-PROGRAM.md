...
## Production risk decision register

[Production Risk Policy v1](PRODUCTION-RISK-POLICY-V1.md) records all 14 discussed aspects and distinguishes owner-confirmed decisions, proposed baselines and open activation evidence. Proposed values do not override this program until explicitly accepted; confirmed amendments retain their stated precedence.
...
The owner-provided experimental capital is IDR 500,000 total in the shared pool. The 5% net monthly objective is a target, not a promise or sizing input. The owner selected an absolute IDR 100,000 drawdown from the cash-flow-adjusted peak equity, regardless of capital size. This supersedes the previous 20% initial-capital loss rule. Percentage drawdown remains a reported metric, not this trigger. Report calendar-month performance against the 5% target and evaluate rolling 90-day results alongside it. The target is not 5% per 90 days. Neither reporting window replaces forward qualification of at least 90 days AND 100 closed forward trades per unchanged candidate.

Illustration only: IDR 2,500 risk (0.5% of IDR 500,000), a 2% stop and an assumed 0.6% combined cost/slippage allowance imply approximately IDR 96,154 notional before rounding and other constraints. Neither 0.5% risk nor 0.6% costs is an approved live default. The previous 0.5% versus 1% comparison remains historical research context. The owner subsequently selected an absolute IDR 4,000 per-trade ceiling for policy v1 qualification, with IDR 10,000 global and IDR 8,000 BTC/ETH/SOL cluster open/pending risk ceilings and IDR 15,000 daily-loss brake. These are approved defaults to test, not activation evidence.
...
### Dashboard defaults and absolute drawdown response
...
| Reporting target/window | 5% net per calendar month plus rolling 90-day evaluation, after trading costs; server operating costs excluded; no forced trading or shorter qualification |
| Absolute portfolio drawdown | IDR 100,000 below adjusted peak equity; floor = adjusted peak equity minus IDR 100,000 |
| Profit use | Reinvest during the evidence-building phase; negative months trigger evaluation, not automatic shutdown when risk and operational gates remain satisfied |
| Threshold action | Controlled close-out; subsequent trading after a drawdown breach requires separately approved new risk period and explicit resume |
| Modeled risk per trade | IDR 4,000 maximum; agent may request less |
| Open and pending risk | IDR 10,000 global; IDR 8,000 combined BTC/ETH/SOL cluster |
| Daily-loss brake | IDR 15,000 from UTC day-opening flow-adjusted equity; latch no-new-entry, eligible exits remain managed |
| Position/exposure capacity | One position per pair; at most two occupied live pair slots; 25% current equity notional per pair and 50% total deployed, including pending entries |

- Policy v1 numerical defaults above are owner-approved for evaluation before activation. All limits apply together; the BTC/ETH/SOL cluster ceiling is the effective IDR 8,000 ceiling for the selected agents. Pending orders reserve cash, risk, exposure and pair slots; partial fill and remaining order on the same pair occupy one slot. Confirmed fills transfer reserved risk to position risk atomically; confirmed cancellation releases only the reconciled unfilled remainder. Daily-stop reset behavior and complete cost/risk valuation remain open in the decision register; no automatic resume is implied.
- PM-08 compares reconciled cash plus trustworthy marked asset value minus estimated remaining exit costs against the capital floor, using Decimal and counting costs once. At or below the floor triggers the policy. The absolute allowance remains IDR 100,000: peaks of IDR 500,000 / 600,000 / 1,000,000 yield floors of IDR 400,000 / 500,000 / 900,000. A new valid trading-equity peak raises the floor; trading losses never lower it. Persist the peak and breach state across restart; prices rebounding after a breach do not clear the close-out intent. Missing marks/costs block entry and disclose unavailable evaluation, never assume zero or safe.
- External deposits/withdrawals require a reviewed peak-adjustment rule before new-policy activation. Compare peak and current equity on the same cash-flow-adjusted basis; deposits cannot fabricate trading profit or repair a drawdown, and withdrawals cannot fabricate trading losses. Preserve cash-flow attribution and historical losses; manual resume, edits and restart cannot silently reset the peak. This packet does not supply an unapproved adjustment formula.
...
- Manual resume must pass the applicable risk period floor, risk, release, health and reconciliation checks. After drawdown close-out, a separately approved new risk period is required before resume; ordinary resume cannot override the breached old-period floor or reset its peak. Controlled close-out is a portfolio-risk action, distinct from candidate replacement draining and from operational HALT. HALT continues to prohibit writes when its gates fail; eligible close-out work waits durably.
- API-04/UI-03 reuse allocation proposal/decision and existing candidate composition/release flows. Expose backend defaults, draft values, active revision, absolute drawdown allowance, adjusted peak equity, active floor, distance to the floor, unset required fields, validation issues, affected positions and activation outcome. Editing supported agent parameters creates a new candidate draft for existing evaluation/release; no direct live agent-parameter patch route is admitted.
- Default/draft/active values stay distinct. Approval binds exact validated bytes and current revisions; stale approval requires a new proposal, identical retries return one result. Widening risk requires new shared-capital evidence. Lower limits block affected entries without implicit liquidation unless the separately approved peak-equity-floor close-out policy is triggered. Existing positions retain candidate-bound exits; central close-out policy can override waiting for a strategy exit signal without rewriting that candidate.

### Risk periods after a drawdown breach

- A drawdown breach remains attached to its original risk period. After close-out completes, the owner may explicitly approve a new period; this is a distinct audited decision, not ordinary resume, automatic reset, or an edit to the old period peak.
- PM-09 must establish completed close-out: no bot-owned/adopted positions, no unresolved orders or reservations, zero UNKNOWN, and fresh full reconciliation. Persist the prior period terminal record with its peak, opening/closing equity, flows, realized losses, fill links, policy/release identities and incident-review reference. Existing accounting remains authoritative; period records reference it rather than create a second ledger.
- PM-08 prepares the new period from actual reconciled equity, not the original IDR 500,000 default. Bind a new period ID and predecessor ID to the exact portfolio revision, proposed initial peak, candidate/release and risk-policy identities. The IDR 100,000 allowance remains the default; all mandatory risk settings and applicable release/qualification evidence must be satisfied.
- API-04 reuses allocation proposal/decision infrastructure with an explicit new-risk-period purpose. Review includes incident disposition, previous-period loss, cumulative results and the new risk envelope. Require production.control, reason, request ID and expected revision; approval binds exact bytes. Changed equity/state invalidates the proposal; duplicate retries create at most one new period and conflicting retries reject.
- Creating a period never edits the old peak or clears historical loss. It does not itself submit orders or resume trading. An explicit resume must then pass existing release, recovery, risk, health and reconciliation gates. Restart or an ordinary policy edit cannot open a new period.
- UI-03 distinguishes resume from starting a new risk period. Show active-period and cumulative all-period results together; preserve the full ledger/event chain. Monthly and rolling 90-day reporting cross period boundaries and never restart because a risk period changed. Deposits/withdrawals are not trading PnL; months without closed trades can still contain unrealized PnL and costs and must not be forced to zero.
...
## Operator absence requirement

The owner may not inspect alerts for 8-12 hours, including sleep. Production incident handling must not rely on immediate manual intervention. This is an unattended-operation requirement, not tolerated downtime or a guarantee of exit execution while the host/network is unavailable.

OPS-01 and QA-03 must qualify existing safe responses to stale feeds, exchange disconnection, process death and uncertain orders with operator acknowledgement absent for up to 12 hours. Preserve entry blocks, durable recovery/close-out intent, single-writer authority and manual resume gates. No automatic failover, automatic resume, broader order authority or new notification service is admitted by this amendment. Alert channel, delivery/escalation policy and detailed outage responses are the next planning topic; existing gates remain until qualified. Additional operational acceptance is pending, not covered by historical PASS evidence.
