...
Run blocking publication/SQLite work through one bounded writer thread per owning store coordinator, preserving single-writer transactions; do not call synchronous fsync on the transport/watchdog event loop. Bound pending submissions before executor entry (an executor's default internal queue is not the admission boundary). Feature/rule updates execute in bounded cooperative slices; a candidate that repeatedly exceeds its evaluation budget is paused. The shared inference subprocess handles native numerical calls. Queue backpressure and missed deadlines remain visible even if disk or CPU delays scheduling.
