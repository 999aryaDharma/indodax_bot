...
For a book publication, the source key is channel + verified provider epoch + offset (or a local source-session boundary when epoch is unavailable); semantic content is the ordered pair/side/price/quantity levels. Existing level source IDs, local `book_session_id`, SNAPSHOT/UPDATE receive role and receive-derived timestamps remain provenance, excluded from this semantic comparison. Thus replay of the same provider publication after reconnect does not become conflicting content solely because local bookkeeping changed. Trade identity retains the existing pair/trade ID and the verified source-epoch namespace; compare price/quantities/side/provider time. Never deduplicate unrelated channel messages merely because their price matches.
...
**Durable trigger contract:** source events and evaluation requests have distinct identities. The scheduler publishes an internal `CanonicalMarketEvent` with `event_type=EVALUATION_TRIGGER` into the same durable feed journal before calling the kernel. This is a derived runtime record, not an additional Indodax payload/channel. Its observation binds `trigger_policy_ref`, `scope_ref` (the immutable input-selection/deployment scope), `trigger_key` (UTC slot or source event/feature-change identity), `as_of_source_cursor`, source market-event refs and feature snapshot refs. Provider epoch/offset/sequence fields are null; venue provenance is inherited by reference, while `source=runtime_scheduler`. The journal assigns its local sequence after the referenced sources.

Its event ID hashes the versioned trigger policy, scope and trigger key; those fields form the unique scheduling key. The immutable envelope additionally binds the selected as-of cursor and snapshot hashes. Re-emitting the same scheduling key must retrieve that committed envelope; different snapshots under that key are a conflict, not a new evaluation. Two periodic slots sharing exactly the same FeatureSnapshot have different event IDs. Consumers with an identical scope/policy may share the trigger; different candidate runtime plans still derive distinct intents under the existing intent-ID rule.

`RuntimeKernel.process` and ADR-007 `prepare_event` receive this trigger envelope's ID/sequence; `CandidateRuntime.evaluate` derives intent IDs from that same ID. Market provenance and reusable feature/prediction identities remain separate references. Raw market events update marks/exits/market-dependent state once; entry/model-trigger evaluation occurs on the derived trigger only. A trigger must not advance closed-bar counters or repeat source-event exit updates. Revalidate input freshness, current namespace state and health before committing its decision.

Scheduler recovery first reuses already journaled triggers and their PREPARED/DECIDED/ACKNOWLEDGED execution state, without reselecting newer snapshots or rerunning a committed decision. A trigger journaled but not yet prepared is processed only if its live deadline still permits it; otherwise commit an explicit skipped outcome. A prepared-but-undecided trigger also rechecks its original deadline; DECIDED work follows the existing outbox recovery rules rather than disappearing on expiry. Slots that elapsed while no trigger was durably scheduled are recorded as missed coverage and never backfilled as live evaluation. Durable per-policy scheduling watermarks advance with journaled trigger/skip records; clock jumps fail clock guard before scheduling.
...
- Tests/failures: same model across candidates loads once; same deterministic request predicts once; version/preprocessing/schema differences do not reuse; corrupt/untrusted artifact rejects; finite outputs; memory/load reservation/eviction/pinned overflow; timeout worker kill/recycle; no training import. Two UTC slots sharing one feature snapshot yield distinct trigger/intent identities while prediction reuse remains valid; retry of one slot cannot rebind newer input. Crash after trigger journal commit and before/after PREPARED, DECIDED and ACKNOWLEDGED preserves one effect, original source refs and deadline/skip semantics. Periodic/candle triggers never produce backdated forward decisions or duplicate bar-held/exit updates.
